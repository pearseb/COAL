subroutine read_int_2d(file, variable, nx, ny, data, title)

! Purpose
! -------
! Generic routine to read a two-dimensional integer array from a netCDF file.
!
! Arguments
! --------
...
! 2008 Feb 5	Steven Phipps	Original version
  ...
  integer, dimension(nx, ny), intent(out) :: data
  ...
  ! Get data
  status = nf_get_var_int(ncid, varid, data)
  if (status /= nf_noerr) then
    write (*, *)
