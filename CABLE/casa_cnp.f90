...
!              1.308011639,1.0/
  ...
  xnplimit(:)  = 1.0
! Q.Zhang test: 23/09/2011
  xNPuptake(:)     = min(xnuptake(:), 1.0)
  if (icycle>2) xNPuptake(:)     = min(xnuptake(:), xpuptake(:))
! end test 23/09/2011
  do np =1, mp
...
!  PRINT *, 'Starting xratesoil; mp = ', mp
  ...
  IF(casamet%iveg2(npt)/=icewater) THEN
    ...
    IF (veg%iveg(npt) == cropland) xkwater(npt)=1.0
...
!  calculate gross mineralisation
   ...
   IF(casamet%iveg2(nland)/=icewater) THEN
      DO j=1,mlitter
         xFluxNlittermin(nland) = xFluxNlittermin(nland) + casaflux%klitter(nland,j) * casapool%Nlitter(nland,j)
      ENDDO
      DO k=1,msoil
         xFluxNsoilmin(nland)   = xFluxNsoilmin(nland)   + casaflux%ksoil(nland,k)   * casapool%Nsoil(nland,k)
      ENDDO

!  calculate N immobilisation from L to S and S to S
      DO kk=1,msoil
      DO j=1,mlitter    ! immobilisation from litter to soil
           xFluxNsoilimm(nland) = xFluxNsoilimm(nland) - casaflux%fromLtoS(nland,kk,j) &
                                * casaflux%klitter(nland,j) * casapool%Clitter(nland,j) &
                                * casapool%ratioNCsoil(nland,kk)
      ENDDO
      DO k=1,msoil      ! immobilisation from soil to soil
         IF(k.ne.kk) THEN
            xFluxNsoilimm(nland) = xFluxNsoilimm(nland) - casaflux%fromStoS(nland,kk,k) &
                                 * casaflux%ksoil(nland,k) &
                                 * casapool%Csoil(nland,k) * casapool%ratioNCsoil(nland,kk)
         ENDIF
      ENDDO
      ENDDO
   ENDIF
...
! If pool size larger than xClitter, turnover rate will not constraied by Nsoilmin.
...
! PRINT *, 'after where construct in casa_xkN'
 DO j=1,mlitter
...
SUBROUTINE phenology(iday,phen)
  IMPLICIT NONE
