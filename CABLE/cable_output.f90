...
MODULE output_module
  ...
  USE define_dimensions, ONLY: r_1,r_2,i_d, &
       ms,nrb,ncs,ncp,mland,mp,mvtype,mstype
  USE define_types
  USE io_variables
  USE physical_constants, ONLY: emleaf,sboltz,emsoil
  USE checks_module, ONLY: mass_balance,energy_balance,ranges
  USE write_module
  USE netcdf
  IMPLICIT NONE
  PRIVATE
  PUBLIC open_output_file,write_output,close_output_file,create_restart
  INTEGER(i_d) :: ncid_out ! output data netcdf file ID
  REAL(r_1) :: missing_value = -999999.0 ! for netcdf output
  TYPE out_varID_type ! output variable IDs in netcdf file
     INTEGER(i_d) :: SWdown,LWdown,Wind,Wind_E,PSurf,Tair,Qair,Rainf, &
          ...
          VegT,Ebal,Wbal,AutoResp,LeafResp,HeteroResp,GPP,NPP,LAI,ECanop, &
          TVeg,ESoil,CanopInt,SnowDepth,HVeg,HSoil,Rnet,tvar
  END TYPE out_varID_type
  TYPE(out_varID_type) :: ovid ! netcdf variable IDs for output variables
  TYPE(parID_type) :: opid ! netcdf variable IDs for output variables
  TYPE output_temporary_type
     REAL(KIND=4),POINTER,DIMENSION(:) :: SWdown  ! 6 downward short-wave radiation [W/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: LWdown   ! 7 downward long-wave radiation [W/m2]
     ...
     REAL(KIND=4),POINTER,DIMENSION(:) :: Tair   ! 11 surface air temperature [K]
     ...
     REAL(KIND=4),POINTER,DIMENSION(:) :: Wind_N ! 15 surface wind speed, N component [m/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: Wind_E ! 16 surface wind speed, E component [m/s]
     ...
     REAL(KIND=4),POINTER,DIMENSION(:) :: Evap     ! 22 total evapotranspiration [kg/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: Ewater ! 23 evap. from surface water storage [kg/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: ESoil    ! 24 bare soil evaporation [kg/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: TVeg     ! 25 vegetation transpiration [kg/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: ECanop   ! 26 interception evaporation [kg/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: PotEvap  ! 27 potential evapotranspiration [kg/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: ACond    ! 28 aerodynamic conductance [m/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: SoilWet  ! 29 total soil wetness [-] 
     REAL(KIND=4),POINTER,DIMENSION(:) :: Albedo   ! 30 albedo [-] 
     REAL(KIND=4),POINTER,DIMENSION(:) :: VegT    ! 31 vegetation temperature [K]
     REAL(KIND=4),POINTER,DIMENSION(:,:) :: SoilTemp  ! 32 av.layer soil temperature [K]
     REAL(KIND=4),POINTER,DIMENSION(:,:) :: SoilMoist ! 33 av.layer soil moisture [kg/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: Qs           ! 34 surface runoff [kg/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: Qsb          ! 35 subsurface runoff [kg/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: DelSoilMoist ! 36 change in soilmoisture (sum layers) [kg/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: DelSWE       ! 37 change in snow water equivalent [kg/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: DelIntercept ! 38 change in interception storage [kg/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: SnowT        ! 39 snow surface temp [K]
     REAL(KIND=4),POINTER,DIMENSION(:) :: BaresoilT    ! 40 surface bare soil temp [K]
     REAL(KIND=4),POINTER,DIMENSION(:) :: AvgSurfT     ! 41 Average surface temperature [K]
     REAL(KIND=4),POINTER,DIMENSION(:) :: RadT         ! 42 Radiative surface temperature [K]
     REAL(KIND=4),POINTER,DIMENSION(:) :: SWE          ! 43 snow water equivalent [kg/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: RootMoist    ! 44 root zone soil moisture [kg/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: CanopInt     ! 45 total canopy water storage [kg/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: NEE          ! 46 net ecosystem exchange [umol/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: NPP          ! 47 net primary production of C by veg [umol/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: GPP          ! 48 gross primary production C by veg [umol/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: AutoResp     ! 49 autotrophic respiration [umol/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: LeafResp     ! 51 autotrophic respiration [umol/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: HeteroResp   ! 50 heterotrophic respiration [umol/m2/s]
     REAL(KIND=4),POINTER,DIMENSION(:) :: SnowDepth    ! actual depth of snow in [m]
     ! Non-Alma variables
     REAL(KIND=4),POINTER,DIMENSION(:) :: Rnet         ! net absorbed radiation [W/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: HVeg         ! sensible heat from vegetation [W/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: HSoil        ! sensible heat from soil [W/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: Ebal         ! cumulative energy balance [W/m2]
     REAL(KIND=4),POINTER,DIMENSION(:) :: Wbal         ! cumulative water balance [W/m2]
...
CONTAINS
  ...
  SUBROUTINE open_output_file(dels,soil,veg,bgc,rough)
    ...
    INTEGER(i_d) :: xID,yID,zID,radID,soilID,soilcarbID,plantcarbID,tID,&
         landID,patchID !,surftypeID ! dimension IDs
    INTEGER(i_d) :: latID,lonID ! time,lat,lon variable ID
    INTEGER(i_d) :: xvID,yvID   ! coordinate variable IDs for GrADS readability
    !    INTEGER(i_d) :: surffracID         ! surface fraction varaible ID
    ...
    ELSE IF(output%grid=='land'.OR. &
       ...
       ok = NF90_DEF_DIM(ncid_out,'land',mland,landID) ! number of land points
       IF (ok /= NF90_NOERR) CALL nc_abort &
    ...
    IF (ok /= NF90_NOERR) CALL nc_abort &
         (ok,'Error writing x coordinate variable (GrADS) units in output file. '// &
         '(SUBROUTINE open_output_file)')
    ...
    IF (ok /= NF90_NOERR) CALL nc_abort &
         (ok,'Error writing y coordinate variable (GrADS) units in output file. '// &
         '(SUBROUTINE open_output_file)')
    ...
    IF(output%met.OR.output%LWdown) THEN
       CALL define_ovar(ncid_out, &
            ovid%LWdown,'LWdown','W/m^2','Downward longwave radiation', &
            patchout%LWdown,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%LWdown(mp))
       out%LWdown = 0.0 ! initialise
    END IF
    IF(output%met.OR.output%Tair) THEN
       CALL define_ovar(ncid_out, &
            ovid%Tair,'Tair','K','Surface air temperature', &
            patchout%Tair,'ALMA',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Tair(mp))
       out%Tair = 0.0 ! initialise
    END IF
    IF(output%met.OR.output%Rainf) THEN
       CALL define_ovar(ncid_out, &
            ovid%Rainf,'Rainf','kg/m^2/s','Rainfall+snowfall', &
            patchout%Rainf,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Rainf(mp))
       out%Rainf = 0.0 ! initialise
    END IF
    IF(output%met.OR.output%Snowf) THEN
       CALL define_ovar(ncid_out, &
            ovid%Snowf,'Snowf','kg/m^2/s','Snowfall', &
            patchout%Snowf,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Snowf(mp))
       out%Snowf = 0.0 ! initialise
    END IF
    IF(output%met.OR.output%Qair) THEN
       CALL define_ovar(ncid_out, &
            ovid%Qair,'Qair','kg/kg','Surface specific humidity', &
            patchout%Qair,'ALMA',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Qair(mp))
       out%Qair = 0.0 ! initialise
    END IF
    IF(output%met.OR.output%Wind) THEN
       CALL define_ovar(ncid_out,&
            ovid%Wind,'Wind','m/s','Scalar surface wind speed', &
            patchout%Wind,'ALMA',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Wind(mp))
       out%Wind = 0.0 ! initialise
    END IF
    IF(output%met.OR.output%PSurf) THEN
       CALL define_ovar(ncid_out, &
            ovid%PSurf,'PSurf','hPa','Surface air pressure', &
            patchout%PSurf,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%PSurf(mp))
       out%PSurf = 0.0 ! initialise
    END IF
    IF(output%met.OR.output%CO2air) THEN
       CALL define_ovar(ncid_out, &
            ovid%CO2air,'CO2air','ppmv','Surface air CO2 concentration', &
            patchout%CO2air,'ALMA',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%CO2air(mp))
       out%CO2air = 0.0 ! initialise
    END IF
    ! Define surface flux variables in output file and allocate temp output vars:
    IF(output%flux.OR.output%Qle) THEN
       CALL define_ovar(ncid_out,ovid%Qle, &
            'Qle','W/m^2','Surface latent heat flux',patchout%Qle,'dummy', &
            xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Qle(mp))
       out%Qle = 0.0 ! initialise
    END IF
    IF(output%flux.OR.output%Qh) THEN
       CALL define_ovar(ncid_out,ovid%Qh, &
            'Qh','W/m^2','Surface sensible heat flux',patchout%Qh,'dummy', &
            xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Qh(mp))
       out%Qh = 0.0 ! initialise
    END IF
    IF(output%flux.OR.output%Qg) THEN
       CALL define_ovar(ncid_out,ovid%Qg, &
            'Qg','W/m^2','Surface ground heat flux',patchout%Qg,'dummy', &
            xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Qg(mp))
       out%Qg = 0.0 ! initialise
    END IF
    IF(output%flux.OR.output%Qs) THEN
       CALL define_ovar(ncid_out,ovid%Qs, &
            'Qs','kg/m^2/s','Surface runoff',patchout%Qs,'dummy', &
            xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Qs(mp))
       out%Qs = 0.0 ! initialise
    END IF
    IF(output%flux.OR.output%Qsb) THEN
       CALL define_ovar(ncid_out,ovid%Qsb, &
            'Qsb','kg/m^2/s','Subsurface runoff',patchout%Qsb,'dummy', &
            xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Qsb(mp))
       out%Qsb = 0.0 ! initialise
    END IF
    IF(output%flux.OR.output%Evap) THEN
       CALL define_ovar(ncid_out,ovid%Evap, &
            'Evap','kg/m^2/s','Total evapotranspiration',patchout%Evap,'dummy', &
            xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Evap(mp))
       out%Evap = 0.0 ! initialise
    END IF
    IF(output%flux.OR.output%ECanop) THEN
       CALL define_ovar(ncid_out,ovid%Ecanop, &
            'ECanop','kg/m^2/s','Wet canopy evaporation',patchout%ECanop,'dummy', &
            xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%ECanop(mp))
       out%ECanop = 0.0 ! initialise
    END IF
    IF(output%flux.OR.output%TVeg) THEN
       CALL define_ovar(ncid_out,ovid%TVeg, &
            'TVeg','kg/m^2/s','Vegetation transpiration',patchout%TVeg,'dummy', &
            xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%TVeg(mp))
       out%TVeg = 0.0 ! initialise
    END IF
    IF(output%flux.OR.output%ESoil) THEN
       CALL define_ovar(ncid_out,ovid%ESoil, &
            'ESoil','kg/m^2/s','Evaporation from soil',patchout%ESoil,'dummy', &
            xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%ESoil(mp))
       out%ESoil = 0.0 ! initialise
    END IF
    IF(output%flux.OR.output%HVeg) THEN
       CALL define_ovar(ncid_out,ovid%HVeg, &
            'HVeg','W/m^2','Sensible heat from vegetation',patchout%HVeg,'dummy', &
            xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%HVeg(mp))
       out%HVeg = 0.0 ! initialise
    END IF
    IF(output%flux.OR.output%HSoil) THEN
       CALL define_ovar(ncid_out,ovid%HSoil, &
            'HSoil','W/m^2','Sensible heat from soil',patchout%HSoil,'dummy', &
            xID,yID,zID,landID,patchID,tID)
    ...
    IF(output%flux.OR.output%carbon.OR.output%NEE) THEN
       CALL define_ovar(ncid_out, &
            ovid%NEE,'NEE','umol/m^2/s','Net ecosystem exchange of CO2', &
            patchout%NEE,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%NEE(mp))
    ...
    IF(output%soil.OR.output%SoilMoist) THEN
       CALL define_ovar(ncid_out, &
            ovid%SoilMoist,'SoilMoist','m^3/m^3','Average layer soil moisture', &
            patchout%SoilMoist,'soil',xID,yID,zID,landID,patchID,soilID,tID)
       ALLOCATE(out%SoilMoist(mp,ms))
       out%SoilMoist = 0.0 ! initialise
    END IF
    IF(output%soil.OR.output%SoilTemp) THEN
       CALL define_ovar(ncid_out, &
            ovid%SoilTemp,'SoilTemp','K','Average layer soil temperature', &
            patchout%SoilTemp,'soil',xID,yID,zID,landID,patchID,soilID,tID)
       ALLOCATE(out%SoilTemp(mp,ms))
       out%SoilTemp = 0.0 ! initialise
    END IF
    IF(output%soil.OR.output%BaresoilT) THEN
       CALL define_ovar(ncid_out, &
            ovid%BaresoilT,'BaresoilT','K','Bare soil temperature', &
            patchout%BaresoilT,'dummy',xID,yID,zID,landID,patchID,tID)
    ...
    IF(output%snow.OR.output%SWE) THEN
       CALL define_ovar(ncid_out,ovid%SWE, &
            'SWE','kg/m^2','Snow water equivalent', &
            patchout%SWE,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%SWE(mp))
       out%SWE = 0.0 ! initialise
    END IF
    IF(output%snow.OR.output%SnowT) THEN
       CALL define_ovar(ncid_out,ovid%SnowT, &
            'SnowT','K','Snow surface temperature', &
            patchout%SnowT,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%SnowT(mp))
       out%SnowT = 0.0 ! initialise
    END IF
    IF(output%snow.OR.output%SnowDepth) THEN
       CALL define_ovar(ncid_out, &
            ovid%SnowDepth,'SnowDepth','m','Snow depth', &
            patchout%SnowDepth,'dummy',xID,yID,zID,landID,patchID,tID)
    ...
    IF(output%radiation.OR.output%SWnet) THEN
       CALL define_ovar(ncid_out,ovid%SWnet, &
            'SWnet','W/m^2','Net shortwave radiation absorbed by surface', &
            patchout%SWnet,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%SWnet(mp))
       out%SWnet = 0.0 ! initialise
    END IF
    IF(output%radiation.OR.output%LWnet) THEN
       CALL define_ovar(ncid_out,ovid%LWnet, &
            'LWnet','W/m^2','Net longwave radiation absorbed by surface', &
            patchout%LWnet,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%LWnet(mp))
       out%LWnet = 0.0 ! initialise
    END IF
    IF(output%radiation.OR.output%Rnet) THEN
       CALL define_ovar(ncid_out,ovid%Rnet, &
            'Rnet','W/m^2','Net radiation absorbed by surface', &
            patchout%Rnet,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Rnet(mp))
       out%Rnet = 0.0 ! initialise
    END IF
    IF(output%radiation.OR.output%Albedo) THEN
       CALL define_ovar(ncid_out, &
            ovid%Albedo,'Albedo','-','Surface albedo', &
            patchout%Albedo,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Albedo(mp))
       out%Albedo = 0.0 ! initialise
    END IF
    IF(output%radiation.OR.output%RadT) THEN
       CALL define_ovar(ncid_out,ovid%RadT, &
            'RadT','K','Radiative surface temperature', &
            patchout%RadT,'dummy',xID,yID,zID,landID,patchID,tID)
    ...
    IF(output%veg.OR.output%VegT) THEN
       CALL define_ovar(ncid_out,ovid%VegT, &
            'VegT','K','Average vegetation temperature', &
            patchout%VegT,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%VegT(mp))
       out%VegT = 0.0 ! initialise
    END IF
    IF(output%veg.OR.output%CanopInt) THEN
       CALL define_ovar(ncid_out,ovid%CanopInt, &
            'CanopInt','kg/m^2','Canopy intercepted water storage', &
            patchout%CanopInt,'dummy',xID,yID,zID,landID,patchID,tID)
    ...
    IF(output%balances.OR.output%Ebal) THEN
       CALL define_ovar(ncid_out,ovid%Ebal, &
            'Ebal','W/m^2','Cumulative energy imbalance', &
            patchout%Ebal,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%Ebal(mp))
       out%Ebal = 0.0 ! initialise
    END IF
    IF(output%balances.OR.output%Wbal) THEN
       CALL define_ovar(ncid_out,ovid%Wbal, &
            'Wbal','kg/m^2','Cumulative water imbalance', &
            patchout%Wbal,'dummy',xID,yID,zID,landID,patchID,tID)
    ...
    IF(output%carbon.OR.output%AutoResp) THEN
       CALL define_ovar(ncid_out, &
            ovid%AutoResp,'AutoResp','umol/m^2/s','Autotrophic respiration', &
            patchout%AutoResp,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%AutoResp(mp))
       out%AutoResp = 0.0 ! initialise
    END IF
    IF(output%carbon.OR.output%LeafResp) THEN
       CALL define_ovar(ncid_out, &
            ovid%LeafResp,'LeafResp','umol/m^2/s','Leaf respiration', &
            patchout%LeafResp,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%LeafResp(mp))
       out%LeafResp = 0.0 ! initialise
    END IF
    IF(output%carbon.OR.output%HeteroResp) THEN
       CALL define_ovar(ncid_out, &
            ovid%HeteroResp,'HeteroResp','umol/m^2/s','Heterotrophic respiration',&
            patchout%HeteroResp,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%HeteroResp(mp))
       out%HeteroResp = 0.0 ! initialise
    END IF
    IF(output%carbon.OR.output%GPP) THEN
       CALL define_ovar(ncid_out,ovid%GPP, &
            'GPP','umol/m^2/s','Gross primary production', &
            patchout%GPP,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%GPP(mp))
       out%GPP = 0.0 ! initialise
    END IF
    IF(output%carbon.OR.output%NPP) THEN
       CALL define_ovar(ncid_out,ovid%NPP, &
            'NPP','umol/m^2/s','Net primary production', &
            patchout%NPP,'dummy',xID,yID,zID,landID,patchID,tID)
       ALLOCATE(out%NPP(mp))
    ...
    IF(output%params.OR.output%albsoil) CALL define_ovar(ncid_out,opid%albsoil,&
         'albsoil','-','Snow free shortwave soil reflectance fraction', &
         patchout%albsoil,radID,'radiation',xID,yID,zID,landID,patchID)
    IF(output%params.OR.output%hc) CALL define_ovar(ncid_out,opid%hc,'hc','m', &
         'Height of canopy',patchout%hc,'real',xID,yID,zID,landID,patchID)
    IF(output%params.OR.output%canst1) CALL define_ovar(ncid_out,opid%canst1, &
         'canst1','mm/LAI','Max water intercepted by canopy', &
    ...
    IF(output%params.OR.output%rpcoef) CALL define_ovar(ncid_out,opid%rpcoef, &
         'rpcoef','1/C','Temperature coef nonleaf plant respiration', &
         patchout%rpcoef,'real',xID,yID,zID,landID,patchID)
    IF(output%params.OR.output%shelrb) CALL define_ovar(ncid_out,opid%shelrb, &
         'shelrb','-','Sheltering factor',patchout%shelrb,'real', &
         xID,yID,zID,landID,patchID)
    ...
    IF(output%params.OR.output%vegcf) CALL define_ovar(ncid_out,opid%vegcf, &
         'vegcf','-','vegcf',patchout%vegcf,'real',xID,yID,zID,landID,patchID)
    ...
    IF(output%params.OR.output%tminvj) CALL define_ovar(ncid_out,opid%tminvj, &
         'tminvj','C','Min temperature for the start of photosynthesis', &
         patchout%tminvj,'real',xID,yID,zID,landID,patchID)
    IF(output%params.OR.output%tmaxvj) CALL define_ovar(ncid_out,opid%tmaxvj, &
         'tmaxvj','C','Max temperature for photosynthesis', &
    ...
    IF(output%params.OR.output%za) THEN
      ...
      CALL define_ovar(ncid_out,opid%za_tq,'za_tq','m', &
         ...
         patchout%za,'real',xID,yID,zID,landID,patchID)
    ENDIF
    IF(output%params.OR.output%ratecp) CALL define_ovar(ncid_out,opid%ratecp, &
         'ratecp','1/year','Plant carbon rate constant', &
         patchout%ratecp,plantcarbID,'plantcarbon',xID,yID,zID,landID,patchID)
    IF(output%params.OR.output%ratecs) CALL define_ovar(ncid_out,opid%ratecs, &
         'ratecs','1/year','Soil carbon rate constant', &
         patchout%ratecs,soilcarbID,'soilcarbon',xID,yID,zID,landID,patchID)
    ...
    IF(output%averaging(1:4)=='user') THEN
       ...
       IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing global detail to '&
            //TRIM(filename%out)// ' (SUBROUTINE open_output_file)')
       ! Convert interval value from hours to time steps (for use in output write):
    ...
    IF((output%params.OR.output%patchfrac) &
         .AND.(patchout%patchfrac.OR.output%patch)) &
         CALL write_ovar(ncid_out,opid%patchfrac,'patchfrac',REAL(patch(:)%frac,4), &
         (/0.0,1.0/),patchout%patchfrac,'real')
    ...
    IF(output%params.OR.output%rhosoil) CALL write_ovar(ncid_out,opid%rhosoil, &
         'rhosoil',REAL(soil%rhosoil,4),ranges%rhosoil,patchout%rhosoil,'real')
    ...
    IF(output%params.OR.output%albsoil) CALL write_ovar(ncid_out,opid%albsoil, &
         'albsoil',REAL(soil%albsoil,4),ranges%albsoil,patchout%albsoil, &
         'radiation')
    IF(output%params.OR.output%canst1) CALL write_ovar(ncid_out,opid%canst1, &
         'canst1',REAL(veg%canst1,4),ranges%canst1,patchout%canst1,'real')
    ...
    IF(output%params.OR.output%rpcoef) CALL write_ovar(ncid_out,opid%rpcoef, &
         'rpcoef',REAL(veg%rpcoef,4),ranges%rpcoef,patchout%rpcoef,'real')
    IF(output%params.OR.output%shelrb) CALL write_ovar(ncid_out,opid%shelrb, &
         'shelrb',REAL(veg%shelrb,4),ranges%shelrb,patchout%shelrb,'real')
    ...
    IF(output%params.OR.output%tminvj) CALL write_ovar(ncid_out,opid%tminvj, &
         'tminvj',REAL(veg%tminvj,4),ranges%tminvj,patchout%tminvj,'real')
    IF(output%params.OR.output%tmaxvj) CALL write_ovar(ncid_out,opid%tmaxvj, &
         'tmaxvj',REAL(veg%tmaxvj,4),ranges%tmaxvj,patchout%tmaxvj,'real')
    ...
    IF(output%params.OR.output%xalbnir) CALL write_ovar(ncid_out,opid%xalbnir, &
         'xalbnir',REAL(veg%xalbnir,4),ranges%xalbnir,patchout%xalbnir,'real')
    ...
    ENDIF
    IF(output%params.OR.output%ratecp) CALL write_ovar(ncid_out,opid%ratecp, &
         'ratecp',SPREAD(REAL(bgc%ratecp,4),1,mp),ranges%ratecp, &
         patchout%ratecp,'plantcarbon')! no spatial dim at present
    IF(output%params.OR.output%ratecs) CALL write_ovar(ncid_out,opid%ratecs, &
         'ratecs',SPREAD(REAL(bgc%ratecs,4),1,mp),ranges%ratecs, &
  ...
  !=====================================================================
  SUBROUTINE write_output(ktau,dels,met,canopy, &
       ssoil,rad,bal,air,soil,veg)
    ...
    TYPE (veg_parameter_type),INTENT(IN) :: veg ! vegetation parameters
    REAL(r_2),DIMENSION(1) :: timetemp ! temporary variable for storing time value
    LOGICAL :: writenow ! write to output file during this time step?
    INTEGER(i_d),SAVE :: out_timestep ! counter for output time steps
    INTEGER(i_d),SAVE :: out_month ! counter for output month
    ...
    ELSE IF(output%averaging(1:4)=='user'.OR.output%averaging(1:2)=='da') THEN
       ...
       IF(MOD(ktau,output%interval)==0) THEN ! i.e.ktau divisible by interval
    ...
    ELSE IF(output%averaging(1:2)=='mo') THEN ! write monthly averages to file
       ...
       IF(leaps) THEN
          ...
          IF(((ANY(MOD(met%year,4)==0).AND.ANY(MOD(met%year,100)/=0)).OR. & 
               (ANY(MOD(met%year,4)==0).AND.ANY(MOD(met%year,400)==0))) &
                                ! Clause below ensures not triggered on last day of non-leap year
               .AND.ANY(met%doy>2.0)) THEN
             IF(ANY((lastdayl*24*3600/INT(dels))==ktau)) THEN ! last time step of month
          ...
          ELSE ! not currently a leap year
             IF(ANY((lastday*24*3600/INT(dels))==ktau)) THEN ! last time step of month
                ! increment output month counter
                out_month = MOD(out_month,12) + 1 ! can only be 1 - 12
                ! write to output file this time step 
                writenow = .TRUE.
    ...
    IF(output%met.OR.output%SWdown) THEN
       ...
       out%SWdown = out%SWdown + REAL(met%fsd,4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%SWdown = out%SWdown/REAL(output%interval,4)
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%SWdown,'SWdown',out%SWdown, &
               ranges%SWdown,patchout%SWdown,'default',met)
    ...
    IF(output%met.OR.output%LWdown) THEN
       ...
       IF(writenow) THEN
          ...
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%LWdown,'LWdown',out%LWdown, &
               ranges%LWdown,patchout%LWdown,'default',met)
          ! Reset temporary output variable:
          out%LWdown = 0.0
       END IF
    END IF
    ...
    IF(output%met.OR.output%Rainf) THEN
       ...
       IF(writenow) THEN
          ...
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%Rainf,'Rainf',out%Rainf, &
               ranges%Rainf,patchout%Rainf,'default',met)
          ! Reset temporary output variable:
          out%Rainf = 0.0
       END IF
    END IF
    ...
    IF(output%met.OR.output%Snowf) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%Snowf = out%Snowf + REAL(met%precip_s/dels,4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%Snowf = out%Snowf/REAL(output%interval,4)
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%Snowf,'Snowf',out%Snowf, &
               ranges%Snowf,patchout%Snowf,'default',met)
          ! Reset temporary output variable:
          out%Snowf = 0.0
       END IF
    END IF
    ...
    IF(output%met.OR.output%PSurf) THEN
       ...
       IF(writenow) THEN
          ...
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%PSurf,'PSurf',out%PSurf, &
               ranges%PSurf,patchout%PSurf,'default',met)
          ! Reset temporary output variable:
          out%PSurf = 0.0
       END IF
    END IF
    ...
    IF(output%met.OR.output%CO2air) THEN
       ...
       IF(writenow) THEN
          ...
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%CO2air,'CO2air',OUT%CO2air, &
               ranges%CO2air,patchout%CO2air,'ALMA',met)
          ! Reset temporary output variable:
          out%CO2air = 0.0
       END IF
    END IF
    ...
    IF(output%flux.OR.output%Qs) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%Qs = out%Qs + ssoil%rnof1/dels
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%flux.OR.output%Qsb) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%Qsb = out%Qsb + REAL(ssoil%rnof2/dels,4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%flux.OR.output%ECanop) THEN
       ...
       IF(writenow) THEN
          ...
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%ECanop,'ECanop',out%ECanop, &
               ranges%ECanop,patchout%ECanop,'default',met)
          ! Reset temporary output variable:
          out%ECanop = 0.0
       END IF
    END IF
    ...
    IF(output%flux.OR.output%ESoil) THEN
       ...
       IF(writenow) THEN
          ...
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%ESoil,'ESoil',out%ESoil, &
               ranges%ESoil,patchout%ESoil,'default',met)
          ! Reset temporary output variable:
          out%ESoil = 0.0
       END IF
    END IF
    ...
    IF(output%flux.OR.output%HSoil) THEN
       ...
       IF(writenow) THEN
          ...
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%HSoil,'HSoil',out%HSoil, &
               ranges%HSoil,patchout%HSoil,'default',met)
          ! Reset temporary output variable:
          out%HSoil = 0.0
       END IF
    END IF
    ...
    IF(output%soil.OR.output%SoilMoist) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%SoilMoist = out%SoilMoist + REAL(ssoil%wb,4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%soil.OR.output%SoilTemp) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%SoilTemp = out%SoilTemp + REAL(ssoil%tgg, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%SoilTemp = out%SoilTemp/REAL(output%interval,4)
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%SoilTemp,'SoilTemp',out%SoilTemp, &
               ranges%SoilTemp,patchout%SoilTemp,'soil',met)
          ! Reset temporary output variable:
          out%SoilTemp = 0.0
       END IF
    END IF
    ...
    IF(output%soil.OR.output%BaresoilT) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%BaresoilT = out%BaresoilT + REAL(ssoil%tgg(:,1),4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%snow.OR.output%SWE) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%SWE = out%SWE + REAL(ssoil%snowd,4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%snow.OR.output%SnowT) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%SnowT = out%SnowT + REAL(ssoil%tggsn(:,1),4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%SnowT = out%SnowT/REAL(output%interval,4)
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%SnowT,'SnowT',out%SnowT, &
               ranges%SnowT,patchout%SnowT,'default',met)
          ! Reset temporary output variable:
          out%SnowT = 0.0
       END IF
    END IF
    ...
    IF(output%snow.OR.output%SnowDepth) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%SnowDepth = out%SnowDepth + REAL(SUM(ssoil%sdepth,2),4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%radiation.OR.output%SWnet) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%SWnet = out%SWnet + &
            REAL(SUM(rad%qcan(:,:,1),2) + SUM(rad%qcan(:,:,2),2) + rad%qssabs, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%radiation.OR.output%Rnet) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%Rnet = out%Rnet + REAL(met%fld-sboltz*emleaf*canopy%tv**4 *(1-rad%transd) &
            -rad%flws*rad%transd+SUM(rad%qcan(:,:,1),2)+SUM(rad%qcan(:,:,2),2) &
            +rad%qssabs, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%radiation.OR.output%Albedo) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%Albedo = out%Albedo + REAL((rad%albedo(:,1) + rad%albedo(:,2))*0.5,4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%radiation.OR.output%RadT) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%RadT = out%RadT + REAL((((1.0-rad%transd)*emleaf*sboltz*canopy%tv**4 &
            +rad%transd*emsoil*sboltz*((1-ssoil%isflag)*ssoil%tgg(:,1) &
            +ssoil%isflag*ssoil%tggsn(:,1)) ** 4)/sboltz)**0.25, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%carbon.OR.output%GPP) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%GPP = out%GPP + REAL((-1.0*canopy%fpn+canopy%frday)/1.201E-5, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%carbon.OR.output%AutoResp) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%AutoResp = out%AutoResp + REAL((canopy%frp+canopy%frday)/1.201E-5, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
    ...
    IF(output%carbon.OR.output%HeteroResp) THEN
       ...
       IF(writenow) THEN
          ...
          ! Write value to file:
          CALL write_ovar(out_timestep,ncid_out,ovid%HeteroResp,'HeteroResp', &
               out%HeteroResp,ranges%HeteroResp,patchout%HeteroResp, &
               'default',met)
          ! Reset temporary output variable:
          out%HeteroResp = 0.0
       END IF
  ...
  !====================================================================
  SUBROUTINE close_output_file(bal,air, bgc, canopy, &
       met, rad, rough, soil, ssoil, sum_flux, veg)
  ...
  !==========================================================================
  SUBROUTINE create_restart(logn,ktau, &
       dels,soil,veg,ssoil,canopy,rough,rad,bgc,bal)
! added mvtype and mstype to define_dimensions (BP sep2010)
!  SUBROUTINE create_restart(logn,ktau, &
!       dels,soil,veg,ssoil,canopy,rough,rad,bgc,bal,mvtype,mstype)
    ...
    INTEGER(i_d), INTENT(IN) :: logn ! log file number
    INTEGER(i_d), INTENT(IN) :: ktau ! time step number (at end of run)
    REAL(r_1), INTENT(IN) :: dels ! time step size
    TYPE (soil_parameter_type),INTENT(IN) :: soil ! soil parameters
    TYPE (veg_parameter_type),INTENT(IN)  :: veg  ! vegetation parameters
...
!    INTEGER, INTENT(IN) :: mstype
    ...
    INTEGER(i_d) :: ncid_restart ! netcdf restart file ID
    !    REAL(r_1), POINTER,DIMENSION(:,:) :: surffrac ! fraction of each surf type
    CHARACTER(LEN=10) :: todaydate, nowtime ! used to timestamp netcdf file
    INTEGER(i_d) :: dummy ! dummy argument in subroutine call
    INTEGER(i_d) :: mlandID,mpID,radID,soilID,napID,&
         soilcarbID,plantcarbID,tID,snowID ! dimension IDs
    !    INTEGER(i_d) :: mlandID,surftypeID,patchID,radID,soilID,&
    !         soilcarbID,plantcarbID,tID,snowID ! dimension IDs
    INTEGER(i_d) :: tvarID,latID,lonID !,surffracID ! time,lat,lon variable ID
    INTEGER(i_d) :: tggID,wbID,wbiceID,tssID,ssdnnID,ssdnID,osnowdID, &
         ...
         cplantID,csoilID,tradID,albedoID
    ...
    IF (ok /= NF90_NOERR) CALL nc_abort &
         ...
         '(SUBROUTINE create_restart)')
    ok = NF90_PUT_ATT(ncid_restart,napID,'long_name','Number of active patches')
    IF (ok /= NF90_NOERR) CALL nc_abort &
    ...
    CALL define_ovar(ncid_restart,smassID,'smass','kg/m^2', &
         ...
         .TRUE.,snowID,'snow',0,0,0,mpID,dummy,.TRUE.)
    CALL define_ovar(ncid_restart,sdepthID,'sdepth','m','Snow layer depth', &
         .TRUE.,snowID,'snow',0,0,0,mpID,dummy,.TRUE.)
    ...
    CALL define_ovar(ncid_restart,canstoID,'cansto','mm', &
         'Canopy surface water storage',.TRUE.,'real',0,0,0,mpID,dummy,.TRUE.)
    ...
    CALL define_ovar(ncid_restart,fesID,'fes','W/m^2', &
         'Latent heat flux from soil',.TRUE.,'real',0,0,0,mpID,dummy,.TRUE.)
    CALL define_ovar(ncid_restart,fhsID,'fhs','W/m^2', &
         'Sensible heat flux from soil',.TRUE.,'real',0,0,0,mpID,dummy,.TRUE.)
    ...
    CALL define_ovar(ncid_restart,rpid%ssat,'ssat','-', &
         'Fraction of soil volume which is water @ saturation',.TRUE.,&
         'real',0,0,0,mpID,dummy,.TRUE.)
    CALL define_ovar(ncid_restart,rpid%sfc,'sfc','-', &
         'Fraction of soil volume which is water @ field capacity',.TRUE.,&
         'real',0,0,0,mpID,dummy,.TRUE.)
    CALL define_ovar(ncid_restart,rpid%swilt,'swilt','-', &
         'Fraction of soil volume which is water @ wilting point',.TRUE.,&
         'real',0,0,0,mpID,dummy,.TRUE.)
    ...
    CALL define_ovar(ncid_restart,rpid%sucs,'sucs','m', &
         'Suction @ saturation',.TRUE.,'real',0,0,0,mpID,dummy,.TRUE.)
    ...
    CALL define_ovar(ncid_restart,rpid%rhosoil,'rhosoil','kg/m^3', &
         'Density of soil minerals',.TRUE.,'real',0,0,0,mpID,dummy,.TRUE.)
    ...
    CALL define_ovar(ncid_restart,rpid%albsoil,'albsoil','-', &
         'Soil reflectance',.TRUE.,radID,'radiation',0,0,0,mpID, &
         dummy,.TRUE.)
    CALL define_ovar(ncid_restart,rpid%hc,'hc','m', &
         'Height of canopy',.TRUE.,'real',0,0,0,mpID,dummy,.TRUE.)
    ...
    IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing latitude variable to ' &
         //TRIM(filename%restart_out)// '(SUBROUTINE create_restart)')
    ok=NF90_PUT_VAR(ncid_restart,lonID,longitude)
    IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing longitude variable to '&
         //TRIM(filename%restart_out)// '(SUBROUTINE create_restart)')

    ! Write number of active patches for each land grid cell:
    ok=NF90_PUT_VAR(ncid_restart,napID,landpt(:)%nap)
    IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing nap variable to ' &
         //TRIM(filename%restart_out)// '(SUBROUTINE create_restart)')
    ...
    IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing mvtype parameter to ' &
         //TRIM(filename%restart_out)// '(SUBROUTINE create_restart)')
    ok=NF90_PUT_VAR(ncid_restart, rpid%mstype,mstype)
    IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing mstype parameter to ' &
         //TRIM(filename%restart_out)// '(SUBROUTINE create_restart)')

    ! Write parameters:
    CALL write_ovar (ncid_restart,rpid%iveg,'iveg',REAL(veg%iveg,4),ranges%iveg, &
         .TRUE.,'integer',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%isoil,'isoil',REAL(soil%isoilm,4), &
         ranges%isoil,.TRUE.,'integer',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%bch,'bch',REAL(soil%bch,4),ranges%bch, &
         .TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%clay,'clay',REAL(soil%clay,4),ranges%clay, &
         .TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%sand,'sand',REAL(soil%sand,4),ranges%sand, &
         .TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%silt,'silt',REAL(soil%silt,4),ranges%silt, &
         .TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%css,'css',REAL(soil%css,4),ranges%css, &
         .TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%rhosoil,'rhosoil',REAL(soil%rhosoil,4), &
         ranges%rhosoil,.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%hyds,'hyds',REAL(soil%hyds,4),ranges%hyds, &
         .TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%sucs,'sucs',REAL(soil%sucs,4),ranges%sucs, &
         .TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%rs20,'rs20',REAL(soil%rs20,4),ranges%rs20, &
         .TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%ssat,'ssat',REAL(soil%ssat,4),ranges%ssat, &
         .TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%sfc,'sfc',REAL(soil%sfc,4),ranges%sfc, &
         .TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%swilt,'swilt',REAL(soil%swilt,4),ranges%swilt, &
         .TRUE.,'real',.TRUE.)
    ! Soil dimensioned variables/parameters:
    CALL write_ovar (ncid_restart,rpid%froot,'froot', REAL(veg%froot,4),ranges%froot, &
         .TRUE.,'soil',.TRUE.)
    CALL write_ovar (ncid_restart,tggID,'tgg',REAL(ssoil%tgg,4),ranges%SoilTemp, &
         .TRUE.,'soil',.TRUE.)
    CALL write_ovar (ncid_restart,wbID,'wb',ssoil%wb,ranges%SoilMoist, &
         .TRUE.,'soil',.TRUE.)
    CALL write_ovar (ncid_restart,wbiceID,'wbice',ssoil%wbice, &
         ranges%SoilMoist,.TRUE.,'soil',.TRUE.)
    CALL write_ovar (ncid_restart,gammzzID,'gammzz',ssoil%gammzz,&
         (/-99999.0,9999999.0/),.TRUE.,'soil',.TRUE.)
    ! Snow dimensioned variables/parameters:
    CALL write_ovar (ncid_restart,ssdnID,'ssdn',REAL(ssoil%ssdn,4),&
         (/0.0,9999.0/),.TRUE.,'snow',.TRUE.)
    CALL write_ovar (ncid_restart,smassID,'smass',REAL(ssoil%smass,4),&
         (/0.0,9999.0/),.TRUE.,'snow',.TRUE.)
    CALL write_ovar (ncid_restart,sdepthID,'sdepth',REAL(ssoil%sdepth,4),&
         (/0.0,9999.0/),.TRUE.,'snow',.TRUE.)
    CALL write_ovar (ncid_restart,tggsnID,'tggsn',REAL(ssoil%tggsn,4),&
         (/100.0,300.0/),.TRUE.,'snow',.TRUE.)
    ! Other dims
    CALL write_ovar (ncid_restart,albsoilsnID,'albsoilsn', &
         REAL(ssoil%albsoilsn,4),(/0.0,1.0/),.TRUE.,'radiation',.TRUE.)
    ...
    IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing zse parameter to ' &
         //TRIM(filename%restart_out)// '(SUBROUTINE create_restart)')
    ! Single dim:
    CALL write_ovar (ncid_restart,rpid%albsoil,'albsoil',REAL(soil%albsoil,4),&
         ranges%albsoil,.TRUE.,'radiation',.TRUE.)
    ...
    CALL write_ovar (ncid_restart,rpid%xalbnir,'xalbnir',REAL(veg%xalbnir,4),&
         ranges%xalbnir,.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rpid%tmaxvj,'tmaxvj',REAL(veg%tmaxvj,4),&
         ranges%tmaxvj,.TRUE.,'real',.TRUE.)
    ok=NF90_PUT_VAR(ncid_restart,rpid%ratecp,REAL(bgc%ratecp,4))
    IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing ratecp parameter to ' &
         //TRIM(filename%restart_out)// '(SUBROUTINE create_restart)')
    ok=NF90_PUT_VAR(ncid_restart,rpid%ratecs,REAL(bgc%ratecs,4))         
    IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing ratecs parameter to ' &
         //TRIM(filename%restart_out)// '(SUBROUTINE create_restart)')
    ...
    CALL write_ovar (ncid_restart,rpid%za_tq,'za_tq',REAL(rough%za_tq,4),&
         ranges%za,.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,tssID,'tss',REAL(ssoil%tss,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,ssdnnID,'ssdnn',REAL(ssoil%ssdnn,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,osnowdID,'osnowd',REAL(ssoil%osnowd,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,snageID,'snage',REAL(ssoil%snage,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,snowdID,'snowd',REAL(ssoil%snowd,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rtsoilID,'rtsoil',REAL(ssoil%rtsoil,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,isflagID,'isflag',REAL(ssoil%isflag,4), &
         (/-99999.0,9999999.0/),.TRUE.,'integer',.TRUE.)
    CALL write_ovar (ncid_restart,canstoID,'cansto',REAL(canopy%cansto,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,sghfluxID,'sghflux',REAL(canopy%sghflux,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,ghfluxID,'ghflux',REAL(canopy%ghflux,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,runoffID,'runoff',REAL(ssoil%runoff,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rnof1ID,'rnof1',REAL(ssoil%rnof1,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
    CALL write_ovar (ncid_restart,rnof2ID,'rnof2',REAL(ssoil%rnof2,4), &
         (/-99999.0,9999999.0/),.TRUE.,'real',.TRUE.)
