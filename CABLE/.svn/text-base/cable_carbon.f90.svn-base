...
!  SUBROUTINE carbon_pl(dels, soil, ssoil, veg, canopy, bgc, mvtype)
    ! BP added mvtype to the list (dec 2007)
...
!  SUBROUTINE soilcarb(soil, ssoil, veg, bgc, met, canopy, mstype)
...
!   REAL(r_1), DIMENSION(mland*max_vegpatches)		:: tref
!   REAL(r_1), DIMENSION(mland*max_vegpatches)		:: tsoil
...
!         & / den
!    tsoil = veg%froot(:,1) * (ssoil%tgg(:,1) - tfrz)
    ! use 2nd soil layer if extreme conditions in 1st layer causes instability
