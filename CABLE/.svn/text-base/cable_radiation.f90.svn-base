...
MODULE radiation_module
  ...
  USE define_dimensions, ONLY: r_1,nrb,mp,mf,i_d,r_2
  USE math_constants
  USE other_constants, ONLY: gauss_w,refl,taul
  USE define_types
  USE physical_constants, ONLY: sboltz,emleaf,emsoil,capp
  IMPLICIT NONE
...
CONTAINS
  ...
  SUBROUTINE init_radiation(met,rad,veg,canopy)
    ...
    WHERE (xvlai2 > 1e-2) ! vegetated
       ...
       xk = SPREAD(xphi1, 2, 3) / SPREAD(cos3, 1, mp) &
            & + SPREAD(xphi2, 2, 3)
   ...
   WHERE (canopy%vlaiw > 1e-2) ! vegetated
      ...
      rad%extkd = -LOG( SUM( SPREAD(gauss_w, 1, mp) &
   ...
   DO bb = 1, nrb  ! bug-fix (YP nov 2009)
     rad%rhocdf(:,bb) = rhoch(bb) &
           * ( gauss_w(1) * xk(:,1) / (xk(:,1)+rad%extkd(:)) &
             + gauss_w(2) * xk(:,2) / (xk(:,2)+rad%extkd(:)) &
             + gauss_w(3) * xk(:,3) / (xk(:,3)+rad%extkd(:)) )
   ENDDO  ! bug-fix (YP nov 2009)
!   rad%rhocdf = SPREAD(2.0*gauss_w*rhoch, 1, mp) * xk & 
...
!    flpwb = sboltz * (met%tvrad) ** 4   ! YP Nov2009 (fix cold bias problem)
    flwv = emleaf * flpwb
    ...
    WHERE (canopy%vlaiw > 1.e-2)
       ...
       rad%gradis(:,1) = (4.0 * emleaf / (capp * air%rho)) * flpwb &
            & / (met%tk) * rad%extkd &
!            & / (met%tvrad) * rad%extkd &   ! YP Nov2009 (fix cold bias problem)
...
!       rad%gradis(:,2) = (8.0*emleaf/(capp*air%rho)) * flpwb / (met%tvrad) &   ! YP Nov2009 (fix cold bias problem)
