! cable_read.f90
!
! Module containing read routines for CABLE; 
!
! Gab Abramowitz 2009 University of New South Wales, gabsun@gmail.com
!
! Precision of real numbers in readpar interface is forced using REAL(,4) 
! to avoid complication when compiled with -r8
!
! The subroutines in this module are:
!   nc_abort
!   and as part of INTERFACE readpar:
!      readpar_i
!      readpar_r
!      readpar_rd
!      readpar_r2
...
MODULE read_module
  ...
  USE io_variables, ONLY: landpt,exists,land_x,land_y,metGrid
  USE netcdf
  IMPLICIT NONE
  PRIVATE
...
CONTAINS
  ...
  SUBROUTINE readpar_i(ncid,parname,completeSet,var_i,filename, &
    ...
    INTEGER(i_d),DIMENSION(:),INTENT(INOUT) :: var_i ! returned parameter values
    CHARACTER(LEN=*), INTENT(IN) :: filename ! file containing parameter values
    INTEGER(i_d), INTENT(IN) :: npatch ! # of veg patches in parameter's file
    ...
    INTEGER(i_d), INTENT(IN),OPTIONAL :: INpatch
    INTEGER(i_d) :: parID ! parameter's netcdf ID
    INTEGER(i_d) :: pardims ! # dimensions of parameter
    INTEGER(i_d),DIMENSION(1) :: data1i ! temporary for ncdf read in
    INTEGER(i_d),DIMENSION(1,1) :: data2i ! temporary for ncdf read in
    INTEGER(i_d),DIMENSION(:,:),POINTER :: tmp2i ! temporary for ncdf read in
    INTEGER(i_d),DIMENSION(:,:,:),POINTER :: tmp3i ! temporary for ncdf read in
          ...
          ELSE IF(pardims==3) THEN ! i.e. parameter has a patch dimension
             ...
             DO  i=1,mland ! over all land points/grid cells
                ok= NF90_GET_VAR(ncid,parID,tmp3i, &
                     start=(/land_x(i),land_y(i),1/),count=(/1,1,npatch/))
                IF(ok /= NF90_NOERR) CALL nc_abort &
                     (ok,'Error reading '//parname//' in file ' &
                     //TRIM(filename)//' (SUBROUTINE readpar_i)')
                ! Set values for this par for the # patches that exist
             ...
             DEALLOCATE(tmp3i)
          ELSE
             CALL abort('Dimension of '//parname//' parameter in met file unknown.')
          END IF
  ...
  SUBROUTINE readpar_r(ncid,parname,completeSet,var_r,filename,&
    ...
    REAL(KIND=4),DIMENSION(:,:,:),POINTER :: tmp3r ! temporary for ncdf read in
    INTEGER(i_d) :: i ! do loop counter
    ! Check if parameter exists:
    ok = NF90_INQ_VARID(ncid,parname,parID)
    IF(ok /= NF90_NOERR) THEN ! if it doesn't exist
...
!                   ELSE ! ie we're reading single prec. var in netdf file
                      ...
                      ok= NF90_GET_VAR(ncid,parID,tmp1r,start=(/1/),count=(/INpatch/))
...
!                   END IF
                ...
                ELSE ! reading from met file
                   DO i=1,mland ! over all land points/grid cells
                      ...
                      IF(ok /= NF90_NOERR) CALL nc_abort &
                           (ok,'Error reading '//parname//' in file ' &
                           //TRIM(filename)//' (SUBROUTINE readpar_rd)')
                      ! Give single value to all patches if no patch specific info:
                      var_rd(landpt(i)%cstart:landpt(i)%cend)=REAL(data1r(1),r_1)
                   END DO
                END IF
             ELSE IF(pardims==2) THEN ! i.e. parameter has a patch dimension
  ...
  SUBROUTINE readpar_r2(ncid,parname,completeSet,var_r2,filename,&
    ...
    CHARACTER(LEN=*), INTENT(IN) :: filename ! file containing parameter values
    INTEGER(i_d), INTENT(IN) :: npatch ! number of veg patches in file
    CHARACTER(LEN=*), INTENT(IN) :: dimswitch ! indicates dimesnion of parameter
    LOGICAL, INTENT(IN),OPTIONAL :: from_restart ! reading from restart file?
    INTEGER(i_d), INTENT(IN),OPTIONAL :: INpatch
    INTEGER(i_d) :: parID ! parameter's netcdf ID
    ...
    REAL(KIND=4),DIMENSION(:,:),POINTER :: tmp2r ! temporary for ncdf read in
    REAL(KIND=4),DIMENSION(:,:,:),POINTER :: tmp3r ! temporary for ncdf read in
    REAL(KIND=4),DIMENSION(:,:,:,:),POINTER :: tmp4r ! temporary for ncdf read in
    INTEGER(i_d) :: dimctr ! size of non-spatial (2nd) dimension of parameter
...
!    WRITE(45,*) 'Is tgg read here?'
       ...
       ELSEIF(metGrid=='mask') THEN ! Get data from land/sea mask type grid:
          ...
          IF(pardims==3) THEN ! no patch dimension, just x-y + soil grid dimension
             ...
             DO  i=1,mland ! over all land points/grid cells
                ok= NF90_GET_VAR(ncid,parID,tmp3r, &
                     start=(/land_x(i),land_y(i),1/),count=(/1,1,dimctr/))
                IF(ok /= NF90_NOERR) CALL nc_abort &
                     (ok,'Error reading '//parname//' in file ' &
                     //TRIM(filename)//' (SUBROUTINE readpar_r2)')
                ! Set all patches to have the same value for this par:
          ...
          ELSE IF(pardims==4) THEN ! i.e. soil parameter has a patch dimension
             ...
             DO  i=1,mland ! over all land points/grid cells
                ok= NF90_GET_VAR(ncid,parID,tmp4r, &
                     start=(/land_x(i),land_y(i),1,1/),count=(/1,1,npatch,dimctr/))
                IF(ok /= NF90_NOERR) CALL nc_abort &
                     (ok,'Error reading '//parname//' in file ' &
                     //TRIM(filename)//' (SUBROUTINE readpar_r2)')
                DO j=1,dimctr
  ...
  SUBROUTINE readpar_r2d(ncid,parname,completeSet,var_r2d,filename,&
    ...
    CHARACTER(LEN=*), INTENT(IN) :: parname ! name of parameter
    LOGICAL, INTENT(INOUT) :: completeSet ! has every parameter been loaded?
    REAL(r_2),DIMENSION(:,:),INTENT(INOUT) :: var_r2d ! returned parameter value
    CHARACTER(LEN=*), INTENT(IN) :: filename ! file containing parameter values
    INTEGER(i_d), INTENT(IN) :: npatch ! # of veg patches in parameter's file
    CHARACTER(LEN=*), INTENT(IN) :: dimswitch ! indicates dimesnion of parameter
    LOGICAL, INTENT(IN),OPTIONAL :: from_restart ! reading from restart file?
    ...
    INTEGER(i_d) :: parID ! parameter's netcdf ID
    INTEGER(i_d) :: pardims ! # dimensions of parameter
    REAL(8),DIMENSION(:,:),POINTER :: tmp2rd ! temporary for ncdf read in
    REAL(4),DIMENSION(:,:),POINTER :: tmp2r ! temporary for ncdf read in
    REAL(4),DIMENSION(:,:,:),POINTER :: tmp3r ! temporary for ncdf read in
    REAL(4),DIMENSION(:,:,:,:),POINTER :: tmp4r ! temporary for ncdf read in
    INTEGER(i_d) :: dimctr ! size of non-spatial (2nd) dimension of parameter
    INTEGER(i_d) :: i,j ! do loop counter
    ! Check if parameter exists:
    ok = NF90_INQ_VARID(ncid,parname,parID)
...
!                DEALLOCATE(tmp2rd)
  ...
  SUBROUTINE redistr_i(INpatch,nap,in_i,out_i,parname)
    ...
    INTEGER(i_d),     INTENT(IN)  :: nap(INpatch)
    INTEGER(i_d),     INTENT(IN)  :: in_i(INpatch)
    INTEGER(i_d),     INTENT(OUT) :: out_i(mp)
    CHARACTER(LEN=*), INTENT(IN)  :: parname ! name of parameter

    ! local variables    
!    REAL(r_1)    :: ave_r
    INTEGER(i_d) :: ii, jj, npt
...
!      out_i(landpt(ii)%cstart:landpt(ii)%cend) = INT(ave_r)
  ...
  SUBROUTINE redistr_r(INpatch,nap,in_r,out_r,parname)
    ...
    INTEGER(i_d),     INTENT(IN)  :: INpatch
    INTEGER(i_d),     INTENT(IN)  :: nap(INpatch)
    REAL(r_1),        INTENT(IN)  :: in_r(INpatch)
    REAL(r_1),        INTENT(OUT) :: out_r(mp)
    CHARACTER(LEN=*), INTENT(IN)  :: parname ! name of parameter
  ...
  SUBROUTINE redistr_r2(INpatch,nap,in_r2,out_r2,parname,dim2)
    IMPLICIT NONE
    INTEGER(i_d),     INTENT(IN)  :: INpatch
    INTEGER(i_d),     INTENT(IN)  :: dim2
    INTEGER(i_d),     INTENT(IN)  :: nap(INpatch)
    REAL(r_1),        INTENT(IN)  :: in_r2 (INpatch,dim2)
    REAL(r_1),        INTENT(OUT) :: out_r2(mp,dim2)
    CHARACTER(LEN=*), INTENT(IN)  :: parname ! name of parameter

    ! local variables
    REAL(r_1)    :: ave_r2(dim2)
    INTEGER(i_d) :: ii, jj, npt
