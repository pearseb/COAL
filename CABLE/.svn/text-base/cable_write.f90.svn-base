! cable_write.f90
!
! Writing routine for CABLE land surface scheme offline netcdf driver; 
! most are called from cable_output.f90
!
! Gab Abramowitz 2009 University of New South Wales gabsun@gmail.com
!
! Single precision netcdf writes are forced to single precision here
! (using REAL(,4)) in case of compilation with -r8 
!
! Subroutines in this module:
!   [part of INTERFACE define_ovar:]
!     define_output_variable_r1
!     define_output_variable_r2
!     define_output_parameter_r1
!     define_output_parameter_r2
!   [part of INTERFACE write_ovar:]
...
MODULE write_module
  USE abort_module
  USE define_dimensions, ONLY:r_1,r_2,i_d,mland,ms,nrb, &
       ncp,ncs,msn,mp
  USE define_types
  USE io_variables, ONLY: landpt, patch, max_vegpatches, &
       parID_type, metGrid, land_x, land_y, logn, output, &
  ...
  PUBLIC define_ovar, write_ovar,otmp1,otmp1l,otmp2lt,otmp2xy,otmp2lp,&
       ...
       otmp3lpt,otmp3lst, &
       otmp3lsnt,otmp3lrt,otmp3lpct,otmp3lsct,otmp3xyp,otmp3xys, &
       otmp3xypc,otmp3xysc,otmp3lps,otmp3lppc,otmp3lpsc,otmp3xysf, &
       otmp3lpr,otmp3lpsn,otmp4xypt,otmp4xyzt,otmp4xyst,&
       otmp4xysnt,otmp4xyrt,otmp4xypct,otmp4xysct,otmp4lpst,otmp4lpsnt, &
       otmp4lprt,otmp4lpsct,otmp4lppct,otmp4xyps,otmp4xyppc,otmp4xypsc, &
       otmp5xypst,otmp5xypsnt,otmp5xyprt,otmp5xyppct,otmp5xypsct
  ...
  REAL(r_1),POINTER,DIMENSION(:) :: otmp1,otmp1l
  REAL(r_1),POINTER,DIMENSION(:,:) :: otmp2lt,otmp2xy,otmp2lp,&
       otmp2ls,otmp2lpc,otmp2lsc,otmp2lsf,otmp2lr,otmp2lsn
  REAL(r_1),POINTER,DIMENSION(:,:,:) :: otmp3xyt,otmp3lpt,otmp3lst, &
       otmp3lsnt,otmp3lrt,otmp3lpct,otmp3lsct,otmp3xyp,otmp3xys, &
       otmp3xypc,otmp3xysc,otmp3lps,otmp3lppc,otmp3lpsc,otmp3xysf, &
       otmp3lpr,otmp3lpsn,otmp3xyr
  REAL(r_1),POINTER,DIMENSION(:,:,:,:) :: otmp4xypt,otmp4xyzt,otmp4xyst,&
       otmp4xysnt,otmp4xyrt,otmp4xypct,otmp4xysct,otmp4lpst,otmp4lpsnt, &
       otmp4lprt,otmp4lpsct,otmp4lppct,otmp4xyps,otmp4xyppc,otmp4xypsc, &
       otmp4xypr
  REAL(r_1),POINTER,DIMENSION(:,:,:,:,:) :: otmp5xypst,otmp5xypsnt,&
       otmp5xyprt,otmp5xyppct,otmp5xypsct
  REAL(r_1) :: ncmissingr = -1.0e+33
  INTEGER(i_d) :: ncmissingi = -9999999
  INTEGER(i_d) :: ok ! netcdf file read status

CONTAINS

  SUBROUTINE define_output_variable_r1(ncid,varID,vname,&
       vunits,longname,writepatch,dimswitch,xID,yID,zID,landID,patchID,tID)
    ! Subroutine for defining a real valued 1D variable
    INTEGER(i_d), INTENT(IN) :: ncid ! netcdf file ID
    INTEGER(i_d), INTENT(OUT) :: varID ! variable's netcdf ID
    ...
    INTEGER(i_d), INTENT(IN) :: xID,yID,zID,landID,patchID,tID ! netcdf dimension IDs 
    ...
    IF(output%grid(1:3)=='mas'.OR. &
      ...
      IF((writepatch.OR.output%patch).AND.(.NOT.output%grid(1:3)=='ALM')) THEN
        ...
        ok=NF90_DEF_VAR(ncid,vname,NF90_FLOAT,(/xID,yID,patchID,tID/),varID) 
      ...
      ELSE ! only grid point values, no patch-specific info
        ...
        IF(dimswitch=='ALMA'.AND.output%grid(1:3)=='ALM') THEN
          ...
          ok=NF90_DEF_VAR(ncid,vname,NF90_FLOAT,(/xID,yID,zID,tID/),varID) 
          ! If not already allocated, allocate a temporary storage variable
          ! of this dim:
    ...
    ELSE IF(output%grid(1:3)=='lan' &
      ...
      IF(writepatch.OR.output%patch) THEN
        ...
        ok=NF90_DEF_VAR(ncid,vname,NF90_FLOAT,(/landID,patchID,tID/),varID)
        IF (ok /= NF90_NOERR) CALL nc_abort &
             (ok,'Error defining '//vname//' variable in output file. '// &
             '(INTERFACE define_ovar)')
        ! If not already allocated, allocate a temporary storage variable
        ! of this dim:
        IF(.NOT.ASSOCIATED(otmp3lpt)) ALLOCATE(otmp3lpt(mland,max_vegpatches,1))
      ELSE ! only grid point values, no patch-specific info
        WRITE(logn,*) 'Writing '//vname// &
  ...
  !========================================================================
  SUBROUTINE define_output_variable_r2(ncid,varID,vname,vunits, &
       longname,writepatch,dimswitch,xID,yID,zID,landID,patchID,othdimID,tID)
    ...
    CHARACTER(LEN=*), INTENT(IN) :: dimswitch ! indicates dimesnion of parameter
    INTEGER(i_d), INTENT(IN) :: xID,yID,zID,landID,patchID,tID ! netcdf dimension IDs 
    INTEGER(i_d), INTENT(IN) :: othdimID ! ID of variable's second dimension
    ...
    IF(output%grid(1:3)=='mas'.OR. &
      ...
      IF((writepatch.OR.output%patch).AND.(.NOT.output%grid(1:3)=='ALM')) THEN
        WRITE(logn,*) 'Writing '//vname// &
             ' to output file using mask grid with patch-specific info'
        ok=NF90_DEF_VAR(ncid,vname,NF90_FLOAT,(/xID,yID,patchID,othdimID, &
             tID/),varID) 
        IF (ok /= NF90_NOERR) CALL nc_abort &
             (ok,'Error defining '//vname//' variable in output file. '// &
             '(INTERFACE define_ovar)')
        ...
        ELSE IF(dimswitch=='plantcarbon') THEN ! other dim is plant carbon pools
          ! If not already allocated, allocate a temporary storage variable
          ! of this dim:
      ...
      ELSE ! only grid point values, no patch-specific info
        WRITE(logn,*) 'Writing '//vname//' to output file using mask grid'
        ok=NF90_DEF_VAR(ncid,vname,NF90_FLOAT,(/xID,yID,othdimID,tID/), &
             varID)
        IF (ok /= NF90_NOERR) CALL nc_abort &
             (ok,'Error defining '//vname//' variable in output file. '// &
    ...
    ELSE IF(output%grid(1:3)=='lan' &
      ...
      IF(writepatch.OR.output%patch) THEN
        WRITE(logn,*) 'Writing '//vname// &
             ' to output file using land grid with patch-specific info'
        ok=NF90_DEF_VAR(ncid,vname,NF90_FLOAT,(/landID,patchID,othdimID, &
             tID/),varID)
        IF (ok /= NF90_NOERR) CALL nc_abort &
             (ok,'Error defining '//vname//' variable in output file. '// &
             '(SUBROUTINE define_output_variable_r2)')
      ...
      ELSE ! only grid point values, no patch-specific info
        WRITE(logn,*) 'Writing '//vname// &
             ' to output file using land grid'
        ok=NF90_DEF_VAR(ncid,vname,NF90_FLOAT,(/landID,othdimID,tID/),varID)
        IF (ok /= NF90_NOERR) CALL nc_abort &
             (ok,'Error defining '//vname//' variable in output file. '// &
             '(SUBROUTINE define_output_variable_r2)')
        IF(dimswitch=='soil') THEN ! other dim is soil
      ...
      END IF
    ELSE
      CALL abort('Unknown grid specification (SUBROUTINE define_output_variable_r2)')
    END IF
    ! Define variable units:
    ok = NF90_PUT_ATT(ncid,varID,'units',vunits)
    IF (ok /= NF90_NOERR) CALL nc_abort &
 ...
 !========================================================================
  SUBROUTINE define_output_parameter_r1(ncid,parID,pname,punits,longname, &
       writepatch,dimswitch,xID,yID,zID,landID,patchID,restart)
    ! Subroutine for defining a real valued 1D parameter (time invariant)
    INTEGER(i_d), INTENT(IN) :: ncid ! netcdf file ID
    INTEGER(i_d), INTENT(OUT) :: parID ! variable's netcdf ID
    ...
    INTEGER(i_d), INTENT(IN) :: xID,yID,zID,landID,patchID ! netcdf dimension IDs
    ...
    ELSE IF(output%grid(1:3)=='lan'.OR.(output%grid(1:3)=='def' &
         .AND.metGrid=='land').OR.PRESENT(restart)) THEN ! land-only grid
       ! Should patch-specific info be written for this variable?
       ...
       ELSE ! only grid point values without patch-specific info UNLESS a 
          ...
          END IF
          IF (ok /= NF90_NOERR) CALL nc_abort &
               (ok,'Error defining '//pname//' variable in output or restart file. '// &
               '(SUBROUTINE define_output_parameter_r1)')
          ! If not already allocated, allocate a temporary storage variable
       ...
       END IF
    ELSE
       CALL abort('Unknown grid specification (SUBROUTINE define_output_parameter_r1)')
    END IF
    ! Define variable units:
    ok = NF90_PUT_ATT(ncid,parID,'units',punits)
    IF (ok /= NF90_NOERR) CALL nc_abort &
         (ok,'Error defining '//pname//' variable attributes in output file. '// &
            '(SUBROUTINE define_output_parameter_r1)')
    ! Define long name:
 ...
 !========================================================================
   SUBROUTINE define_output_parameter_r2(ncid,parID,pname,punits,longname, &
        writepatch,othdimID,dimswitch,xID,yID,zID,landID,patchID,restart)
    ...
    CHARACTER(LEN=*), INTENT(IN) :: dimswitch ! indicates dimesnion of parameter
    INTEGER(i_d), INTENT(IN) :: xID,yID,zID,landID,patchID ! netcdf dimension IDs 
    INTEGER(i_d), INTENT(IN) :: othdimID ! ID of parameter's second dimension
    LOGICAL,INTENT(IN),OPTIONAL :: restart ! are we writing to a restart file?
    ...
    IF((output%grid(1:3)=='mas'.OR. &
       ...
       ! Should patch-specific info be written for this variable
       ! (no patches in ALMA format)?
       IF((writepatch.OR.output%patch).AND.(.NOT.output%grid(1:3)=='ALM') &
            .AND.(dimswitch/='surftype')) THEN
          WRITE(logn,*) 'Writing '//pname// &
               ' to output file using mask grid with patch-specific info'
       ...
       ELSE ! only grid point values, no patch-specific info 
          WRITE(logn,*) 'Writing '//pname//' to output file using mask grid'
          ok=NF90_DEF_VAR(ncid,pname,NF90_FLOAT,(/xID,yID,othdimID/),parID)
          ! If not already allocated, allocate a temporary storage variable
          ! of this dim:
          IF(dimswitch=='soil'.OR.dimswitch=='r2soil') THEN
             IF(.NOT.ASSOCIATED(otmp3xys))ALLOCATE(otmp3xys(xdimsize,ydimsize,ms))
          ELSE IF(dimswitch=='plantcarbon') THEN
          ...
          ELSE IF(dimswitch=='surftype') THEN
             IF(.NOT.ASSOCIATED(otmp3xysf))ALLOCATE(otmp3xysf(xdimsize,ydimsize,4))
          END IF
          IF (ok /= NF90_NOERR) CALL nc_abort &
               (ok,'Error defining '//pname//' variable in output file. '// &
               '(SUBROUTINE define_output_parameter_r2)')
    ...
    ELSE IF(output%grid(1:3)=='lan'.OR.(output%grid(1:3)=='def' &
       ...
       IF((writepatch.OR.output%patch).AND.(dimswitch/='surftype') &
          ...
          ! of this dim:
          IF(dimswitch=='soil'.OR.dimswitch=='r2soil') THEN
             IF(.NOT.ASSOCIATED(otmp3lps)) ALLOCATE(otmp3lps(mland,max_vegpatches,ms))
          ELSE IF(dimswitch=='plantcarbon') THEN
       ...
       END IF
    ELSE
       CALL abort('Unknown grid specification (SUBROUTINE define_output_parameter_r2)')
    END IF
    ! Define variable units:
    ok = NF90_PUT_ATT(ncid,parID,'units',punits)
    IF (ok /= NF90_NOERR) CALL nc_abort &
 ...
 !========================================================================
  SUBROUTINE write_output_variable_r1(ktau,ncid,varID,vname,var_r1,&
    ...
    INTEGER(i_d), INTENT(IN) :: ktau ! current time step #
    INTEGER(i_d), INTENT(IN) :: ncid ! netcdf file ID
    INTEGER(i_d), INTENT(IN) :: varID ! variable's netcdf ID
    CHARACTER(LEN=*), INTENT(IN) :: vname ! name of variable
    REAL(KIND=4),DIMENSION(:),INTENT(IN) :: var_r1 ! variable values
    REAL(r_1),DIMENSION(2),INTENT(IN) :: vrange ! max and min for variable error checking
    LOGICAL, INTENT(IN) :: writepatch ! write patch-specific info for this var?
    CHARACTER(LEN=*), INTENT(IN) :: dimswitch ! indicates dimesnion of parameter
      ...
      ! (no patches in ALMA format)?
       IF((writepatch.OR.output%patch).AND.(.NOT.output%grid(1:3)=='ALM')) THEN
          DO i = 1, mland ! over all land grid points
             ...
             IF(landpt(i)%nap<max_vegpatches) otmp4xypt(land_x(i),land_y(i), &
             ...
             IF(check%ranges) THEN  ! Check ranges:
                DO j=1,landpt(i)%nap ! only check active patches
                   IF((otmp4xypt(land_x(i),land_y(i),j,1)<vrange(1)).OR. &
                        ...
                        ktau,met,otmp4xypt(land_x(i),land_y(i),j,1),vrange,i, &
                        land_x(i),land_y(i))
                END DO
             END IF
          END DO
          ...
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4xypt(:,:,:,1),4), &
               start=(/1,1,1,ktau/),count=(/xdimsize,ydimsize,max_vegpatches,1/))
       ELSE ! only grid point values, no patch-specific info
        ! If this is an ALMA 4D surface variable
        ! AND the user has forced the grid type as ALMA:
        IF(dimswitch=='ALMA'.AND.output%grid(1:3)=='ALM') THEN
          DO i = 1, mland ! over all land grid points
            ! Write to temporary variable (area weighted average across all patches):
             otmp4xyzt(land_x(i),land_y(i),1,1) = SUM(var_r1(landpt(i)%cstart: &
          ...
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4xyzt,4),start=(/1,1,1,ktau/), &
        ...
        ELSE ! normal x-y-t mask grid
          ...
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp3xyt,4),start=(/1,1,ktau/), &
    ...
    ELSE IF(output%grid(1:3)=='lan' &
      ...
      IF(writepatch.OR.output%patch) THEN
        DO i = 1, mland ! over all land grid points
           ! First write data for active patches:
           otmp3lpt(i,1:landpt(i)%nap,1) = var_r1(landpt(i)%cstart:landpt(i)%cend)
      ...
      END IF
    ELSE
      CALL abort('Unknown grid specification (SUBROUTINE write_output_variable_r1)')
    END IF
  ...
  SUBROUTINE write_output_variable_r2(ktau,ncid,varID,vname,var_r2,&
    ...
    CHARACTER(LEN=*), INTENT(IN) :: dimswitch ! indicates dimesnion of parameter
    TYPE(met_type),INTENT(IN) :: met  ! met data
    INTEGER(i_d) :: i,j,k ! do loop counter
    ...
    IF(output%grid(1:3)=='mas'.OR. &
      ...
      IF((writepatch.OR.output%patch).AND.(.NOT.output%grid(1:3)=='ALM')) THEN
        ...
        IF(dimswitch=='soil') THEN ! other dim is soil
          DO i = 1, mland ! over all land grid points
            ! First write data for active patches:
             ...
             ! Then write data for inactive patches (if any) as dummy value:
             IF(landpt(i)%nap<max_vegpatches) otmp5xypst(land_x(i),land_y(i), &
                  (landpt(i)%nap+1):max_vegpatches,:,1) = ncmissingr
            IF(check%ranges) THEN  ! Check ranges for active patches:
              DO j=1,landpt(i)%nap
                DO k=1,ms
                  IF((otmp5xypst(land_x(i),land_y(i),j,k,1)<vrange(1)).OR. &
                       (otmp5xypst(land_x(i),land_y(i),j,k,1)>vrange(2))) &
                       CALL range_abort(vname//' is out of specified ranges!', &
        ...
        ELSE IF(dimswitch=='snow') THEN ! other dim is snow
          DO i = 1, mland ! over all land grid points
            ! First write data for active patches:
             otmp5xypsnt(land_x(i),land_y(i),1:landpt(i)%nap,:,1) &
                   = var_r2(landpt(i)%cstart:landpt(i)%cend,:)
             ! Then write data for inactive patches as dummy value:
             IF(landpt(i)%nap<max_vegpatches) otmp5xypsnt(land_x(i),land_y(i), &
             (landpt(i)%nap+1):max_vegpatches,:,1) = ncmissingr
            IF(check%ranges) THEN  ! Check ranges for active patches:
              DO j=1,landpt(i)%nap
                DO k=1,msn
                  IF((otmp5xypsnt(land_x(i),land_y(i),j,k,1)<vrange(1)).OR. &
                       (otmp5xypsnt(land_x(i),land_y(i),j,k,1)>vrange(2))) &
                       CALL range_abort(vname//' is out of specified ranges!', &
          ...
          DO j=1,max_vegpatches
                DO k=1,msn
                   WHERE(mask/=1) otmp5xypsnt(:,:,j,k,1) = ncmissingr ! not land
                END DO
          END DO
          ! Write data to file:
        ...
        ELSE IF(dimswitch=='radiation') THEN ! other dim is radiation bands
          DO i = 1, mland ! over all land grid points
            ! First write data for active patches:
             otmp5xyprt(land_x(i),land_y(i),1:landpt(i)%nap,:,1) &
                   = var_r2(landpt(i)%cstart:landpt(i)%cend,:)
             ! Then write data for inactive patches as dummy value:
              IF(landpt(i)%nap<max_vegpatches) otmp5xyprt(land_x(i),land_y(i), &
              (landpt(i)%nap+1):max_vegpatches,:,1) = ncmissingr
            IF(check%ranges) THEN  ! Check ranges for active patches:
              DO j=1,landpt(i)%nap
                DO k=1,nrb
                  IF((otmp5xyprt(land_x(i),land_y(i),j,k,1)<vrange(1)).OR. &
                       (otmp5xyprt(land_x(i),land_y(i),j,k,1)>vrange(2))) &
                       CALL range_abort(vname//' is out of specified ranges!', &
          ...
          DO j=1,max_vegpatches
                DO k=1,nrb
                   WHERE(mask/=1) otmp5xyprt(:,:,j,k,1) = ncmissingr ! not land
                END DO
          END DO
          ! Write data to file:
        ...
        ELSE IF(dimswitch=='plantcarbon') THEN ! other dim is plant carbon pools
          DO i = 1, mland ! over all land grid points
            ! First write data for active patches:
             otmp5xyppct(land_x(i),land_y(i),1:landpt(i)%nap,:,1) &
                   = var_r2(landpt(i)%cstart:landpt(i)%cend,:)
             ! Then write data for inactive patches (if any) as dummy value:
              IF(landpt(i)%nap<max_vegpatches) otmp5xyppct(land_x(i),land_y(i), &
                   (landpt(i)%nap+1):max_vegpatches,:,1) = ncmissingr
            IF(check%ranges) THEN  ! Check ranges for active patches:
              DO j=1,landpt(i)%nap
                DO k=1,ncp
                  IF((otmp5xyppct(land_x(i),land_y(i),j,k,1)<vrange(1)).OR. &
                       (otmp5xyppct(land_x(i),land_y(i),j,k,1)>vrange(2))) &
                       CALL range_abort(vname//' is out of specified ranges!', &
          ...
          DO j=1,max_vegpatches
                DO k=1,ncp
                   WHERE(mask/=1) otmp5xyppct(:,:,j,k,1) = ncmissingr ! not land
                END DO
          END DO
          ! Write data to file:
        ...
        ELSE IF(dimswitch=='soilcarbon') THEN ! other dim is soil carbon pools
          DO i = 1, mland ! over all land grid points
            ! First write data for active patches:
             otmp5xypsct(land_x(i),land_y(i),1:landpt(i)%nap,:,1) &
                   = var_r2(landpt(i)%cstart:landpt(i)%cend,:)
             ! Then write data for inactive patches as dummy value:
             IF(landpt(i)%nap<max_vegpatches) otmp5xypsct(land_x(i),land_y(i), &
             (landpt(i)%nap+1):max_vegpatches,:,1) = ncmissingr
            IF(check%ranges) THEN  ! Check ranges for active patches:
              DO j=1,landpt(i)%nap
                DO k=1,ncs
                  IF((otmp5xypsct(land_x(i),land_y(i),j,k,1)<vrange(1)).OR. &
                       (otmp5xypsct(land_x(i),land_y(i),j,k,1)>vrange(2))) &
                       CALL range_abort(vname//' is out of specified ranges!', &
          ...
          DO j=1,max_vegpatches
            DO k=1,ncs
              WHERE(mask/=1) otmp5xypsct(:,:,j,k,1) = ncmissingr ! not land
            END DO
          END DO
          ! Write data to file:
      ...
      ELSE ! only grid point values, no patch-specific info
        ...
        IF(dimswitch=='soil') THEN ! other dim is soil
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4xyst,4),start=(/1,1,1,ktau/), &
        ...
        ELSE IF(dimswitch=='snow') THEN ! other dim is snow
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4xysnt,4),start=(/1,1,1,ktau/), &
        ...
        ELSE IF(dimswitch=='radiation') THEN ! other dim is radiation bands
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4xyrt,4),start=(/1,1,1,ktau/), &
        ...
        ELSE IF(dimswitch=='plantcarbon') THEN ! other dim is plant carbon pools
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4xypct,4),start=(/1,1,1,ktau/), &
        ...
        ELSE IF(dimswitch=='soilcarbon') THEN ! other dim is soil carbon pools
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4xysct,4),start=(/1,1,1,ktau/), &
      ...
      IF((writepatch.OR.output%patch).AND.(.NOT.output%grid(1:3)=='ALM')) THEN
        ...
        IF(dimswitch=='soil') THEN ! other dim is soil
          ...
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4lpst(:,:,:,1),4), &
               start=(/1,1,1,ktau/), count=(/mland,max_vegpatches,ms,1/))
        ELSE IF(dimswitch=='snow') THEN ! other dim is snow
          DO i = 1, mland ! over all land grid points
            ! First write data for active patches:
          ...
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4lpsnt(:,:,:,1),4), &
               start=(/1,1,1,ktau/),count=(/mland,max_vegpatches,msn,1/))
        ELSE IF(dimswitch=='radiation') THEN ! other dim is radiation bands
          DO i = 1, mland ! over all land grid points
            ! First write data for active patches:
          ...
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4lprt(:,:,:,1),4), &
               start=(/1,1,1,ktau/),count=(/mland,max_vegpatches,nrb,1/))
        ELSE IF(dimswitch=='plantcarbon') THEN ! other dim is plant carbon pools
          DO i = 1, mland ! over all land grid points
            ! First write data for active patches:
          ...
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4lppct(:,:,:,1),4), &
               start=(/1,1,1,ktau/),count=(/mland,max_vegpatches,ncp,1/))
        ELSE IF(dimswitch=='soilcarbon') THEN ! other dim is soil carbon pools
          DO i = 1, mland ! over all land grid points
            ! First write data for active patches:
          ...
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp4lpsct(:,:,:,1),4), &
               start=(/1,1,1,ktau/),count=(/mland,max_vegpatches,ncs,1/))
        ELSE
          CALL abort('Variable '//vname// &
               ' defined with unknown dimension switch - '//dimswitch//&
               ' - in INTERFACE write_ovar')
      ...
      ELSE ! only grid point values, no patch-specific info
        ...
        IF(dimswitch=='soil') THEN ! other dim is soil
          DO i = 1, mland ! over all land grid points
            ...
            END IF
          END DO
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp3lst,4),start=(/1,1,ktau/), &
        ...
        ELSE IF(dimswitch=='snow') THEN ! other dim is snow
          DO i = 1, mland ! over all land grid points
            ...
            END IF
          END DO
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp3lsnt,4),start=(/1,1,ktau/), &
        ...
        ELSE IF(dimswitch=='radiation') THEN ! other dim is radiation bands
          DO i = 1, mland ! over all land grid points
            ...
            END IF
          END DO
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp3lrt,4),start=(/1,1,ktau/), &
        ...
        ELSE IF(dimswitch=='plantcarbon') THEN ! other dim is plant carbon pools
          DO i = 1, mland ! over all land grid points
            ...
            END IF
          END DO
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp3lpct,4),start=(/1,1,ktau/), &
        ...
        ELSE IF(dimswitch=='soilcarbon') THEN ! other dim is soil carbon pools
          DO i = 1, mland ! over all land grid points
            ...
            END IF
          END DO
          ok=NF90_PUT_VAR(ncid,varID,REAL(otmp3lsct,4),start=(/1,1,ktau/), &
    ...
    ELSE 
      CALL abort('Unknown grid specification (SUBROUTINE write_output_variable_r2)')
    END IF ! grid type
...
!========================================================================
  SUBROUTINE write_output_parameter_r1(ncid,parID,pname,par_r1,&
    ...
    INTEGER(i_d), INTENT(IN) :: ncid ! netcdf file ID
    INTEGER(i_d), INTENT(IN) :: parID ! variable's netcdf ID
    CHARACTER(LEN=*), INTENT(IN) :: pname ! name of variable
    REAL(KIND=4),DIMENSION(:),INTENT(IN) :: par_r1 ! variable values
    REAL(r_1),DIMENSION(2),INTENT(IN) :: prange ! max and min for variable error checking
    LOGICAL, INTENT(IN) :: writepatch ! write patch-specific info for this var?
    CHARACTER(LEN=*), INTENT(IN) :: dimswitch ! indicates dimesnion of parameter
    LOGICAL,INTENT(IN),OPTIONAL :: restart ! are we writing to a restart file?
    INTEGER(i_d) :: i,j ! do loop counter
    ...
    IF((output%grid(1:3)=='mas'.OR. &
       ...
       IF((writepatch.OR.output%patch).AND.(.NOT.output%grid(1:3)=='ALM')) THEN
          DO i = 1, mland ! over all land grid points
             ...
             ELSE IF(dimswitch(1:1)=='i') THEN
                IF(landpt(i)%nap<max_vegpatches) otmp3xyp(land_x(i),land_y(i), &
                     (landpt(i)%nap+1):max_vegpatches) = ncmissingi
             END IF
          ...
          IF(dimswitch(1:1)=='r') THEN
             ...
             ok=NF90_PUT_VAR(ncid,parID,REAL(otmp3xyp(:,:,:),4), &
                  start=(/1,1,1/),count=(/xdimsize,ydimsize,max_vegpatches/))
          ELSE IF(dimswitch(1:1)=='i') THEN
             ! Fill non-land points with dummy value:
             DO j=1,max_vegpatches
       ...
       ELSE ! only grid point values, no patch-specific info
          ...
          IF(dimswitch(1:1)=='r') THEN
             ...
             ok=NF90_PUT_VAR(ncid,parID,REAL(otmp2xy,4),start=(/1,1/), &
          ...
          END IF
       END IF
    ELSE IF(output%grid(1:3)=='lan' &   ! land only grid (e.g. used for restart)
       ...
       IF((writepatch.OR.output%patch).AND..NOT.PRESENT(restart)) THEN
          DO i = 1, mland ! over all land grid points
             ! First write data for active patches:
             otmp2lp(i,1:landpt(i)%nap) = par_r1(landpt(i)%cstart:landpt(i)%cend)
             ! Then write data for inactive patches as dummy value:
          ...
          ELSE IF(dimswitch(1:1)=='i') THEN
             ok=NF90_PUT_VAR(ncid,parID,INT(otmp2lp(:,:)), &
                  start=(/1,1/),count=(/mland,max_vegpatches/))
          END IF
       ELSE ! only grid point values without patch-specific info UNLESS restart file
    ...
    ELSE
       CALL abort('Unknown grid specification (SUBROUTINE write_output_parameter_r1)')
    END IF  ! mask x-y or land-only grid
    ! Check writing was successful:
    IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing '//pname// &
         ' parameter/variable to output file (SUBROUTINE write_output_parameter_r1)')
    ...
    ELSE ! a 1D double precision time invariant parameter for output file
       ...
       DO i = 1, mland ! over all land grid points
          ...
          tmpout(i,1:landpt(i)%nap) = par_r1d(landpt(i)%cstart:landpt(i)%cend)
           ! Then write data for inactive patches as dummy value:
          IF(landpt(i)%nap<max_vegpatches) &
               tmpout(i,(landpt(i)%nap+1):max_vegpatches) = REAL(ncmissingr,r_2)
          IF(check%ranges) THEN  ! Check ranges over active patches:
             DO j=1,landpt(i)%nap
       ...
       ok=NF90_PUT_VAR(ncid,parID,REAL(tmpout(:,:),8), &
            start=(/1,1/),count=(/mland,max_vegpatches/)) ! write data to file
       DEALLOCATE(tmpout)
       ! Check writing was successful:
       IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing '//pname// &
    ...
    IF((output%grid(1:3)=='mas'.OR. &
      ...
      IF((writepatch.OR.output%patch).AND.(.NOT.output%grid(1:3)=='ALM') &
        ...
        IF(dimswitch=='soil') THEN ! i.e. spatial and soil
          DO i = 1, mland ! over all land grid points
             ...
             ! Then write data for inactive patches as dummy value:
             IF(landpt(i)%nap<max_vegpatches) otmp4xyps(land_x(i),land_y(i), &
                     (landpt(i)%nap+1):max_vegpatches,:) = ncmissingr
            IF(check%ranges) THEN  ! Check ranges:
              DO j=1,landpt(i)%nap
          ...
          ok=NF90_PUT_VAR(ncid,parID,REAL(otmp4xyps(:,:,:,:),4), &
               start=(/1,1,1,1/),count=(/xdimsize,ydimsize,max_vegpatches,ms/))
        ELSE IF(dimswitch=='plantcarbon') THEN ! i.e. spatial and plant carbon
          DO i = 1, mland ! over all land grid points
             ...
             otmp4xyppc(land_x(i),land_y(i),1:landpt(i)%nap,:) &
                  = par_r2(landpt(i)%cstart:landpt(i)%cend,:)
            ! Then write data for inactive patches as dummy value:
             IF(landpt(i)%nap<max_vegpatches) otmp4xyppc(land_x(i),land_y(i), &
                     (landpt(i)%nap+1):max_vegpatches,:) = ncmissingr
            IF(check%ranges) THEN  ! Check ranges:
              DO j=1,landpt(i)%nap
                IF(ANY(otmp4xyppc(land_x(i),land_y(i),j,:)<prange(1)).OR. &
                     ANY(otmp4xyppc(land_x(i),land_y(i),j,:)>prange(2))) THEN
                  WRITE(*,*) 'Parameter '//pname// &
                             ' is set at a value out of specified ranges!'
                  WRITE(*,*) 'Land point # ',i, 'patch #', j
          ...
          ok=NF90_PUT_VAR(ncid,parID,REAL(otmp4xyppc(:,:,:,:),4), &
               start=(/1,1,1,1/),count=(/xdimsize,ydimsize,max_vegpatches,ncp/))
        ELSE IF(dimswitch=='soilcarbon') THEN ! i.e. spatial and soil carbon
          DO i = 1, mland ! over all land grid points
             ...
             otmp4xypsc(land_x(i),land_y(i),1:landpt(i)%nap,:) &
                  = par_r2(landpt(i)%cstart:landpt(i)%cend,:)
             ! Then write data for inactive patches as dummy value:
             IF(landpt(i)%nap<max_vegpatches) otmp4xypsc(land_x(i),land_y(i), &
                     (landpt(i)%nap+1):max_vegpatches,:) = ncmissingr
            IF(check%ranges) THEN  ! Check ranges:
              DO j=1,landpt(i)%nap
                IF(ANY(otmp4xypsc(land_x(i),land_y(i),j,:)<prange(1)).OR. &
                     ANY(otmp4xypsc(land_x(i),land_y(i),j,:)>prange(2)))  THEN
                  WRITE(*,*) 'Parameter '//pname// &
                             ' is set at a value out of specified ranges!'
                  WRITE(*,*) 'Land point # ',i, 'patch #', j
          ...
          ok=NF90_PUT_VAR(ncid,parID,REAL(otmp4xypsc(:,:,:,:),4), &
               start=(/1,1,1,1/),count=(/xdimsize,ydimsize,max_vegpatches,ncs/))
        ELSE IF(dimswitch=='radiation') THEN ! i.e. spatial and soil carbon
          DO i = 1, mland ! over all land grid points
             ...
             otmp4xypr(land_x(i),land_y(i),1:landpt(i)%nap,:) &
                  = par_r2(landpt(i)%cstart:landpt(i)%cend,:)
             ! Then write data for inactive patches as dummy value:
             IF(landpt(i)%nap<max_vegpatches) otmp4xypr(land_x(i),land_y(i), &
                     (landpt(i)%nap+1):max_vegpatches,:) = ncmissingr
            IF(check%ranges) THEN  ! Check ranges:
              DO j=1,landpt(i)%nap
                IF(ANY(otmp4xypr(land_x(i),land_y(i),j,:)<prange(1)).OR. &
                     ANY(otmp4xypr(land_x(i),land_y(i),j,:)>prange(2)))  THEN
                  WRITE(*,*) 'Parameter '//pname// &
                             ' is set at a value out of specified ranges!'
                  WRITE(*,*) 'Land point # ',i, 'patch #', j
          ...
          ok=NF90_PUT_VAR(ncid,parID,REAL(otmp4xypr(:,:,:,:),4), &
               start=(/1,1,1,1/),count=(/xdimsize,ydimsize,max_vegpatches,nrb/))
        ELSE
      ...
      ELSE ! only grid point values, no patch-specific info
        ...
        IF(dimswitch=='soil') THEN ! i.e. spatial and soil
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,parID,REAL(otmp3xys,4),start=(/1,1,1/), &
        ...
        ELSE IF(dimswitch=='plantcarbon') THEN ! i.e. spatial and plant carbon
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,parID,REAL(otmp3xypc,4),start=(/1,1,1/), &
        ...
        ELSE IF(dimswitch=='soilcarbon') THEN ! i.e. spatial and soil carbon
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,parID,REAL(otmp3xysc,4),start=(/1,1,1/), &
        ...
        ELSE IF(dimswitch=='radiation') THEN ! i.e. spatial and soil carbon
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,parID,REAL(otmp3xyr,4),start=(/1,1,1/), &
        ...
        ELSE IF(dimswitch=='surftype') THEN ! i.e. surface fraction 
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,parID,REAL(otmp3xysf,4),start=(/1,1,1/), &
        ...
        END IF
      END IF
    ELSE IF(output%grid(1:3)=='lan' & ! land only grid
            .OR.(output%grid(1:3)=='def'.AND.metGrid=='land') &
            .OR.PRESENT(restart)) THEN
      ...
      IF((writepatch.OR.output%patch).AND.(dimswitch/='surftype') &
        ...
        ELSE IF(dimswitch=='radiation') THEN ! i.e. spatial and radiation bands
          DO i = 1, mland ! over all land grid points
            ! First write data for active patches:
     ...
     ELSE ! Varaible has no patch dimension
      ...
      END IF
    ELSE
      CALL abort('Unknown grid specification (SUBROUTINE write_output_parameter_r2)')
    END IF
    ! Check writing was successful:
    IF(ok/=NF90_NOERR) CALL nc_abort(ok,'Error writing '//pname// &
  ...
  SUBROUTINE write_output_parameter_r2d(ncid,parID,pname,par_r2d,&
    ...
    IF(dimswitch=='soil') THEN ! i.e. spatial and soil
       ...
       ELSE
          ...
          END DO
          ok=NF90_PUT_VAR(ncid,parID,REAL(tmpout(:,:,:),8), &
               start=(/1,1,1/),count=(/mland,max_vegpatches,ms/)) ! write data to file
          DEALLOCATE(tmpout)
       END IF
    ELSE
       CALL abort('Parameter '//pname// &
