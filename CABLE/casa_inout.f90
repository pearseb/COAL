...
!  INTEGER(i_d),              INTENT(IN)    :: mvt
  ...
  INTEGER, DIMENSION(nphen)     :: ivtx(nphen)
...
!  INTEGER(i_d),               INTENT(IN)    :: mvt
  ...
  ! local variables
  INTEGER(i_d), DIMENSION(:,:), ALLOCATABLE :: iso
  REAL(r_1),DIMENSION(:,:), ALLOCATABLE :: annNdep,annNfix,annPwea,annPdust
  REAL(r_1),DIMENSION(:), ALLOCATABLE:: latx, lonx
...
SUBROUTINE casa_init(casamet,casapool,casabal,veg)
...
! end addition (BP may2010)
  ...
  REAL(r_2) :: latz,lonz,fracz,glaiz,slaz
...
!    ENDDO
    ...
    READ(99,*)           ! skip first row recording patch number Q.Zhang @ 04/05/2011

    DO npt =1, mp
      SELECT CASE(icycle)
      CASE(1)
        READ(99,92) nyearz,npz,ivtz,istz,isoz,latz,lonz,fracz,casamet%glai(npt), &
      ...
      END SELECT 
!      PRINT *, npt,nyearz,npz,ivtz,istz,isoz,latz,lonz,fracz,casamet%glai(npt)
    ENDDO
    CLOSE(99)
  ENDIF 
...
SUBROUTINE casa_poolout(ktau,veg,soil,casabiome,casapool,casaflux,casamet, &
  ...
  USE define_dimensions
  USE define_types
  USE io_variables,ONLY: landpt,patch        ! output patchfrac, Q.Zhang 10/10/2011
  USE casadimension
  USE casaparm
  USE casavariable
  USE phenvariable
  ...
  WRITE(nout,*) mp         ! total patch number Q.Zhang 26/10/2011
...
!        casamet%areacell(npt)*(1.0e-9),casamet%glai(npt),       &
        patch(npt)%frac,casamet%glai(npt),       &
...
!    PRINT *, 'calling nuptake'
    ...
    IF (icycle >2) call casa_puptake(veg,xkNlimiting,casabiome,casapool,casaflux,casamet)
  ENDIF 
...
!  call casa_coeffsoil(xklitter,xksoil,veg,soil,casabiome,casaflux,casamet)
!
!  IF (icycle>1) call casa_xkN(xkNlimiting,casapool,casaflux,casamet,veg)
...
!  casapool%Psoillab(:) = MAX(0.0, casapool%Psoillab(:)) ! was 0.1 (Q.Zhang 23/05/2011)
