...
gen_test=function(infiles,sitenames,outtype,plottypes,oldfiles){
  ...
  for(site in 1:length(infiles)){ # for each site
    ...
    # cnlist[9] = '  filename%veg     = \'surface_data/veg_parm_oldVegcf.txt\''
    # cnlist[9] = '  filename%veg     = \'surface_data/veg_parm_new.txt\''
    ...
    # range checking not for WalkerBranch, Hyytiala, LittleWashita and Bondville
...
diurnalflux = function(outtype,sitename,cablefile,obsfile,oldfile){
  ...
  for(j in 1:length(flux)){ # for each flux variable
    ...
    nyears=as.integer(ndays/365) # find # years in data set
    # Read obs:
    if (flux[j] == 'Rnet') {                  # some sites do not have Rnet obs
      if (is.character(onc$var$Rnet$name)) {
        check = TRUE
      } else {
        check = FALSE
    ...
    if (check) {
      ...
      obs_days=array(0.0,c((alllen/tstepinday),tstepinday))
    }
    minFlux=0.0
    maxFlux=0.0
    cavday=array(0.0,c(4,tstepinday))
    oavday=array(0.0,c(4,tstepinday))
    lavday=array(0.0,c(4,tstepinday))
...
#      oavday[1:tstepinday]=0 # initialise
      ...
      for(l in 1:nyears){	
        for(i in 1:tstepinday){
          ...
          cavday[k,i]=cavday[k,i] + sum(cable_days[(stid[k]+(l-1)*365):
                                                   (fnid[k]+(l-1)*365),i])
          lavday[k,i]=lavday[k,i] + sum(lastv_days[(stid[k]+(l-1)*365):
                                                   (fnid[k]+(l-1)*365),i])
          oavday[k,i]=oavday[k,i] + sum(obs_days[(stid[k]+(l-1)*365):
                                                 (fnid[k]+(l-1)*365),i])
        }
        if(k==1){ # i.e. DJF
          # add Dec to Jan/Feb
          for(i in 1:tstepinday){
            cavday[k,i]=cavday[k,i] + sum(cable_days[(stid[k+4]+(l-1)*365):
                                                     (fnid[k+4]+(l-1)*365),i])
            lavday[k,i]=lavday[k,i] + sum(lastv_days[(stid[k+4]+(l-1)*365):
                                                     (fnid[k+4]+(l-1)*365),i])
            oavday[k,i]=oavday[k,i] + sum(obs_days[(stid[k+4]+(l-1)*365):
                                                   (fnid[k+4]+(l-1)*365),i])
          }
        }
      }
      ...
      if(k==1){ # i.e. DJF
        ...
        lavday[k,1:tstepinday]=lavday[k,1:tstepinday]/(90*nyears)
        oavday[k,1:tstepinday]=oavday[k,1:tstepinday]/(90*nyears)
      }else{
        cavday[k,1:tstepinday]=cavday[k,1:tstepinday]/((fnid[k]-stid[k])*nyears)
        lavday[k,1:tstepinday]=lavday[k,1:tstepinday]/((fnid[k]-stid[k])*nyears)
        oavday[k,1:tstepinday]=oavday[k,1:tstepinday]/((fnid[k]-stid[k])*nyears)
      }
      # find the min and max values for plotting
      minFlux=min(cavday[k,1:tstepinday],oavday[k,1:tstepinday],
                  lavday[k,1:tstepinday],Fmin)
      maxFlux=max(cavday[k,1:tstepinday],oavday[k,1:tstepinday],
                  lavday[k,1:tstepinday],Fmax)
    }
    for(k in 1:4){# for each season (DJF, MAM etc)
      ...
      # Plot last version CABLE output:
      lines(xloc,lavday[k,1:tstepinday],lwd=2,col='green')
      # Then plot obs result:
      if (check) lines(xloc,oavday[k,1:tstepinday],lwd=2)
    ...
    rm(cavday,lavday,oavday)
  }
  legend(-2,maxFlux,c('new model','old model','obs'),lty=1,
         col=c('salmon','green','black'),lwd=2,bty="n")
  # Close netcdf files
  close.ncdf(cnc)
  close.ncdf(onc)
