...
# memory (?!) specifications. They are now scaled down to make it work.i
# (BP, Oct 2007)
...
# BP modified this to plot old version for comparison
    
# Compared to CABLEplotsComp.R, this version have
#       check%ranges=.FALSE.
#       spinup=.FALSE.
# and require restart file to be passed in and written to the namelist.
...
gen_test=function(infiles,sitenames,outtype,restartfiles,plottypes,oldfiles){
  ...
  for(site in 1:length(infiles)){ # for each site
    ...
    cnlist[1] = '&cable'
    cnlist[2] = paste('  filename_met = \'',infiles[site],'\'',sep='')
    cnlist[3] = '  filename_out = \'out_cable.nc\''
    cnlist[4] = '  filename_log = \'log_cable.txt\''
    cnlist[5] = paste('  filename_restart_in  = \'',restartfiles[site],'\'',sep='')
    cnlist[6] = '  filename_restart_out = \'./restart_out.nc\''
    cnlist[7] = '  filename_LAI     = \'surface_data/LAI_Monthly_Global.nc\''
    cnlist[8] = '  filename_type    = \'surface_data/VegSoil_Type_Global_igbp.csv\''
    cnlist[9] = '  filename_veg     = \'surface_data/def_veg_params_igbp.txt\''
    # cnlist[9] = '  filename_veg     = \'surface_data/veg_parm_oldVegcf.txt\''
    # cnlist[9] = '  filename_veg     = \'surface_data/veg_parm_new.txt\''
    # cnlist[9] = '  filename_veg     = \'surface_data/def_veg_params.txt\''
    cnlist[10] = '  filename_soil    = \'surface_data/def_soil_params.txt\''
    ...
    # range checking not for LittleWashita and Bondville
    cnlist[25] = '  check%ranges     = .FALSE. ! variable ranges, input and output'
    ...
    system('make') # Will make and run CABLE
    ...
    file.copy('../../restart_out.nc',paste('restart_',sitenames[site],'.nc',sep=''))
    # Produce plots:
    if(any(plottypes=='seasflux')) seasflux(outtype,sitenames[site],
      '../../out_cable.nc',paste('../../',infiles[site],sep=''),oldfiles[site])
...
seasflux=function(outtype='pdf',sitename,cablefile,obsfile,oldfile){
  ...
  lnc=open.ncdf(oldfile,readunlim=FALSE) # open last version CABLE file
  ...
  lastv_monthly=c()    # initialise monthly averages
  for(j in 1:length(flux)){ # for each flux variable
    ...
    lastv_data=get.var.ncdf(lnc,flux[j]) # read last version CABLE output
    # Reshape into column days:
    cable_days=matrix(cable_data,ncol=tstepinday,byrow=TRUE)
    lastv_days=matrix(lastv_data,ncol=tstepinday,byrow=TRUE)
    ...
    for(i in 1:ndays){
      ...
      lavday[i]=mean(lastv_days[i,]) # calc daily average flux
    }	
    ...
    for(l in 1:12){ # for each month
      ...
      for(k in 1:nyears){ # for each year of data set
        ...
        lastv_month = lastv_month +
                      sum(lavday[(month_start[l]+(k-1)*365):
                                 (month_start[l+1]-1 +(k-1)*365) ] )
      }
      # then divide by the total number of days added above:
      cable_monthly[l]=cable_month/(month_length*nyears)
      lastv_monthly[l]=lastv_month/(month_length*nyears)
    }
    max1 = max(cable_monthly)
    max2 = max(lastv_monthly)
    maxcable = max(max1,max2)
    min1 = min(cable_monthly)
    min2 = min(lastv_monthly)
    mincable = min(min1,min2)
    # Read obs: and make sure Rnet data is there
    if (flux[j] == 'Rnet') {
      if (is.character(onc$var$Rnet$name)) {
        check = TRUE
      } else {
        check = FALSE
      }
    ...
    if (check) {
      ...
      # this is for Tumbarumba with data gaps
      for(i in 1:alllen){
        if (obs_data[i] < -9000.0) obs_data[i]=0.0
      }
      ...
      maxobs = max(obs_monthly)
      minobs = min(obs_monthly)
    } else {
      maxobs = 0.0
      minobs = 200.0
    }
    ...
    plot(xloc,cable_monthly,type="l",xaxt="n",xlab='Month',
      ylab=paste('Average',flux[j],'flux',units[j]),lwd=2,col='salmon',
      ylim=c(min(mincable,minobs),
      max(maxcable,maxobs)))
    # Plot last version CABLE output:
    lines(xloc,lastv_monthly,lwd=2,col='green')
    # Then plot obs result:
    if (check)  lines(xloc,obs_monthly,lwd=2)
    axis(1,at=c(2,4,6,8,10,12),labels=c('2','4','6','8','10','12'))
    title(paste(sitename,flux[j])) # add title
  }
  legend(1,max(maxcable,maxobs),c('new model','old model','obs'),lty=1,
         col=c('salmon','green','black'),lwd=2,bty="n")
  # Close netcdf files
  close.ncdf(cnc)
  close.ncdf(onc)
  close.ncdf(lnc)
...
diurnalflux = function(outtype,sitename,cablefile,obsfile,oldfile){
  outfilename=paste(sitename,'Diurnal.',outtype,sep='') # name of plot 
                                                        # if not to screen
  flux=c('NEE','Qle','Qh','Rnet') # outputs to plot and
  units=c('umol/m2/s','W/m2','W/m2','W/m2') # their units
  ...
  cnc=open.ncdf(cablefile,readunlim=FALSE) # open CABLE file
  onc=open.ncdf(obsfile,readunlim=FALSE) # open observed data file
  lnc=open.ncdf(oldfile,readunlim=FALSE) # open last version CABLE output
  # Get timing details (1st 2 time steps of 'time' variable):
  time12=get.var.ncdf(cnc,'time',start=1,count=2) # read CABLE 'time' variable
  timestepsize=time12[2]-time12[1]
  tstepinday=86400/timestepsize # number of time steps in a day
  ...
  for(j in 1:length(flux)){ # for each flux variable
    ...
    for(k in 1:4){# for each season (DJF, MAM etc)
      ...
      lavday=c() # declare
      lavday[1:tstepinday]=0 # initialise
      # sum up fluxes over each year of data set for current season
      for(l in 1:nyears){	
        for(i in 1:tstepinday){
          # calc average flux for each hour
          cavday[i]=cavday[i] + sum(cable_days[(stid[k]+(l-1)*365):
                                               (fnid[k]+(l-1)*365),i])
          lavday[i]=lavday[i] + sum(lastv_days[(stid[k]+(l-1)*365):
                                               (fnid[k]+(l-1)*365),i])
        }
        if(k==1){ # i.e. DJF
          # add Dec to Jan/Feb
          for(i in 1:tstepinday){
            cavday[i]=cavday[i] + sum(cable_days[(stid[k+4]+(l-1)*365):
                                                 (fnid[k+4]+(l-1)*365),i])
            lavday[i]=lavday[i] + sum(lastv_days[(stid[k+4]+(l-1)*365):
                                                 (fnid[k+4]+(l-1)*365),i])
          }
        }
      }
      ...
      if(k==1){ # i.e. DJF
        cavday=cavday/(90*nyears)
        lavday=lavday/(90*nyears)
      }else{
        cavday=cavday/((fnid[k]-stid[k])*nyears)
        lavday=lavday/((fnid[k]-stid[k])*nyears)
      }
      max1 = max(cavday)
      max2 = max(lavday)
      cmax = max(max1,max2)
      ...
      if (check) {
        ...
        for(l in 1:nyears){
          for(i in 1:tstepinday){
            oavday[i]=oavday[i] + sum(obs_days[(stid[k]+(l-1)*365):
                                               (fnid[k]+(l-1)*365),i])
          }
          if(k==1){ # i.e. DJF
            for(i in 1:tstepinday){
              oavday[i]=oavday[i] + sum(obs_days[(stid[k+4]+(l-1)*365):
                                                 (fnid[k+4]+(l-1)*365),i])
            }
          }
        }
        if(k==1){ # i.e. DJF
          oavday=oavday/(90*nyears)
        }else{
          oavday=oavday/((fnid[k]-stid[k])*nyears)
        }
        omax = max(oavday)
        omin = min(oavday)
      } else {
      ...
      # Plot CABLE output result:
      plot(xloc,cavday,type="l",xaxt="n",xlab='Hour of day',
          ylab=paste(flux[j],'flux',units[j]),lwd=2,col='salmon',
          ylim=c(min(cmin,omin),max(cmax,omax)))
      # Plot last version CABLE output:
      lines(xloc,lavday,lwd=2,col='green')
      # Then plot obs result:
      if (check) lines(xloc,oavday,lwd=2)
  ...
  legend(-2,max(cmax,omax),c('new model','old model','obs'),lty=1,
         col=c('salmon','green','black'),lwd=2,bty="n")
  # Close netcdf files
  close.ncdf(cnc)
  close.ncdf(onc)
...
timeseries=function(sitename,cablefile,outtype='png',
                    timestep_start=-1,timestep_stop=-1){
  outfilename=paste(sitename,'Series.',outtype,sep='') # name of plot
                                                       # if not to screen
  if(outtype=='pdf'){
    pdf(file=outfilename,paper='a4r',width=11,height=8)
  }else if(outtype=='ps'){
  ...
  # set layout of plots (12 in 3*4 matrix):
  ...
  legend('topleft',c('1','2','3','4','5','6'),lty=1,
         col=c('green','forestgreen','darkslategrey','deepskyblue4','blue',
         'darkblue'),lwd=0.5)
